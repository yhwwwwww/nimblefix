...
public:
  struct Options
  {
    ...
    // Queue mode binds handler[i] to worker i; co-scheduled drains on worker
    // i's runtime thread, threaded runs one app thread per worker.
  ...
  // Connects synchronously and may block until the TCP dial succeeds or times
  // out.
  auto OpenSession(std::uint64_t session_id, std::string host, std::uint16_t port) -> base::Status;
  // Enqueues the session for a later dial on the runtime worker loop without
  // blocking the caller.
  ...
  [[nodiscard]] auto pending_reconnect_count() const -> std::size_t { return pending_reconnect_count_.load(); }
