...
// state, and ~40 shared method implementations. Subclasses implement the handful
// of virtual methods that differ between acceptor and initiator.
class LiveSessionWorker {
  ...
  protected:
    ...
    class CommandSink final : public session::SessionCommandSink {
      public:
        CommandSink(LiveSessionWorker* owner, std::uint32_t worker_id, std::size_t queue_capacity)
            : owner_(owner), worker_id_(worker_id), queue_(queue_capacity) {}
        ...
        auto LoadSnapshot(std::uint64_t session_id) const
            -> base::Result<session::SessionSnapshot> override {
    ...
    // Merged ActiveSession: acceptor uses routed_worker_id; initiator uses host/port.
    ...
    // Initiator reconnect queue entry (stored in WorkerShardState; always empty for acceptor).
    ...
    // Merged WorkerShardState: acceptor uses active_connections; initiator uses pending_reconnects.
    ...
    virtual auto HandleInboundFrame(
    ...
    // Differs: acceptor calls SendApplication when options_.application == nullptr;
    // initiator skips application_messages in that case.
    virtual auto HandleProtocolEvent(
        WorkerShardState& shard,
    ...
    // Acceptor: must re-find connection after HandleInboundFrame (may migrate shard).
    // Initiator: no migration; simpler re-use of pointer.
    virtual auto ProcessConnection(
        WorkerShardState& shard,
        std::size_t connection_index,
        bool readable,
        std::uint64_t timestamp_ns) -> base::Status = 0;
    ...
    virtual auto CloseConnection(
        WorkerShardState& shard,
        std::size_t connection_index,
        std::uint64_t timestamp_ns) -> void = 0;

    // --- Virtual with default (no-op): initiator overrides ---

    // Process pending reconnect entries in shard; no-op for acceptor.
    virtual auto ProcessPendingReconnects(WorkerShardState& shard, std::uint64_t timestamp_ns)
        -> base::Status;
    ...
    auto DispatchAppMessage(
        ...
        message::MessageView message,
        std::uint64_t timestamp_ns) -> base::Status;
    auto ServiceTimer(WorkerShardState& shard, ConnectionState& connection, std::uint64_t timestamp_ns)
        -> base::Status;
    auto SendFrame(
        ConnectionState& connection,
