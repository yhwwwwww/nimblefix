...
class CompiledMessageDecoder {
  public:
    ...
    [[nodiscard]] auto slot(std::uint8_t index) const -> const CompiledFieldSlot& {
        return slots_[index];
    }
...
/// Constructed once at dictionary load time, shared across all decode calls.
class CompiledDecoderTable {
  public:
    ...
    /// Find the pre-compiled decoder for a message type. Returns nullptr if unknown.
