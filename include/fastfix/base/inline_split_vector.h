...
public:
  ...
  [[nodiscard]] auto size() const -> std::size_t { return inline_size_ + overflow_.size(); }
  ...
  auto clear() -> void
  {
    if constexpr (!std::is_trivially_destructible_v<T>) {
      // Assigning T{} drops resources held by the inline slots before we
      // logically reset the vector.
  ...
  [[nodiscard]] auto back() -> T& { return (*this)[size() - 1U]; }

  [[nodiscard]] auto back() const -> const T& { return (*this)[size() - 1U]; }
