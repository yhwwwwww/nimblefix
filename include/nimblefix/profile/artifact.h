...
public:
  ...
  [[nodiscard]] std::optional<std::string_view> string_at(std::size_t offset) const
  {
    ...
    const auto* terminator = static_cast<const char*>(std::memchr(begin, '\0', static_cast<std::size_t>(end - begin)));
...
public:
  ...
  [[nodiscard]] bool valid() const { return descriptor_ != nullptr; }

  [[nodiscard]] const ArtifactSection& descriptor() const { return *descriptor_; }
  ...
  [[nodiscard]] const Entry& operator[](std::size_t index) const { return entries_[index]; }
...
public:
  ...
  [[nodiscard]] std::span<const ArtifactSection> sections() const { return sections_; }

  [[nodiscard]] std::uint64_t profile_id() const { return header().profile_id; }

  [[nodiscard]] std::uint64_t schema_hash() const { return header().schema_hash; }
...
private:
  ...
  friend auto MakeLoadedProfile(std::shared_ptr<const std::byte> storage,
                                std::size_t size,
...
inline bool
HasArtifactMagic(const ArtifactHeader& header)
{
  return std::memcmp(header.magic, kArtifactMagic.data(), kArtifactMagic.size()) == 0;
}

inline bool
HasLittleEndianTag(const ArtifactHeader& header)
{
  return header.endian_tag == kArtifactEndianLittle;
}

inline auto
MakeLoadedProfile(std::shared_ptr<const std::byte> storage,
                  std::size_t size,
                  const ArtifactHeader* header,
                  std::span<const ArtifactSection> sections) -> LoadedProfile
{
