...
struct FieldRule
{
  ...
  [[nodiscard]] bool required() const { return (flags & static_cast<std::uint32_t>(FieldRuleFlags::kRequired)) != 0; }
};
...
public:
  ...
  [[nodiscard]] const LoadedProfile& profile() const { return profile_; }

  [[nodiscard]] std::size_t field_count() const { return field_defs_.size(); }

  [[nodiscard]] std::size_t message_count() const { return message_defs_.size(); }

  [[nodiscard]] std::size_t group_count() const { return group_defs_.size(); }

  [[nodiscard]] std::span<const FieldDefRecord> fields() const { return field_defs_.entries(); }

  [[nodiscard]] std::span<const MessageDefRecord> messages() const { return message_defs_.entries(); }

  [[nodiscard]] std::span<const GroupDefRecord> groups() const { return group_defs_.entries(); }
  ...
  [[nodiscard]] auto enum_values() const -> std::span<const EnumValueRecord> { return enum_values_.entries(); }
...
private:
  ...
  /// Direct-address table: tag -> index into field_defs_ for tags <
