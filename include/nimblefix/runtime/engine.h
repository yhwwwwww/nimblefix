...
/// Return a CounterpartyConfig to accept the session, or an error Status to
/// reject.
using SessionFactory = std::function<base::Result<CounterpartyConfig>(const session::SessionKey& key)>;

/// A simple factory that accepts sessions matching a whitelist of CompID
/// patterns.
...
public:
  ...
  [[nodiscard]] auto profiles() const -> const ProfileRegistry& { return profiles_; }

  [[nodiscard]] auto runtime() const -> const ShardedRuntime* { return runtime_.has_value() ? &*runtime_ : nullptr; }

  [[nodiscard]] auto mutable_runtime() -> ShardedRuntime* { return runtime_.has_value() ? &*runtime_ : nullptr; }

  [[nodiscard]] auto metrics() const -> const MetricsRegistry& { return metrics_; }

  [[nodiscard]] auto mutable_metrics() -> MetricsRegistry* { return &metrics_; }

  [[nodiscard]] auto trace() const -> const TraceRecorder& { return trace_; }

  [[nodiscard]] auto mutable_trace() -> TraceRecorder* { return &trace_; }

  [[nodiscard]] auto config() const -> const EngineConfig* { return config_.has_value() ? &*config_ : nullptr; }
