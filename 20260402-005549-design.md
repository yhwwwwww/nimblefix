...
- 将协议身份拆分为 transport session profile、application baseline、dialect overlay 与 validation policy 四层。
...
- 协议 profile 描述：传输层版本、应用层基线版本、overlay 合并结果、校验策略。
...
2. application message 字典基线
3. counterparty / venue 的 custom overlay
4. 当前连接采用的校验严格度

若将这四层混成一个“FIXVersion”枚举，会直接带来以下问题：
...
2. `ApplicationDictionaryBaseline`

- 定义应用消息的标准基线字典。
- 例如 FIX 4.2、FIX 4.4、FIX 5.0 SP2。
- 包括字段、消息、组件、group、枚举和 required 关系。

3. `DialectOverlay`

- 定义特定对手方、交易所、经纪商的 custom 扩展。
- 允许增加私有 tag、私有 MsgType、扩展枚举、放宽或收紧字段要求、扩展 group 成员。
- 这层是现实世界对接的关键，不应被当成例外逻辑散落在 session 或 parser 中。

4. `ValidationPolicy`

- 定义当前连接的校验强度。
- 至少建议提供 `strict`、`compatible`、`permissive`、`raw-pass-through` 四档。
- 决定未知 tag、字段顺序、重复字段、缺失非关键字段时的行为。

这四层在会话建立前合成为不可变的 `ResolvedProtocolProfile`，供 codec、session 和 typed API 共同使用。
...
- `BeginString` 同时决定 transport 语义和默认 application baseline。
- 例如 `FIX.4.2` 可映射为：
    - `TransportSessionProfile = FIX42_CLASSIC`
    - `ApplicationDictionaryBaseline = FIX42_STD`

2. 对 FIXT.1.1

- `BeginString=FIXT.1.1` 只决定 transport 语义。
- 应用消息版本由 `DefaultApplVerID` 或消息级 `ApplVerID` 决定。
- 因此一个 session transport profile 可以绑定多个 application baseline。

统一采用这套内部模型后，代码层面无需把 FIXT 当成特例分支，而是按相同 profile 解析流程工作。

### 6.7 Overlay 合并规则

`DialectOverlay` 不能在热路径动态解释。正确做法是在启动期或建连前，把 baseline dictionary 与 overlay 预编译成规范化字典。
...
4. Header / Trailer overlay

- 允许附加 venue 自定义头字段。
- 不允许悄然改变关键 session tag 的核心语义。

合并输出必须是一个 `NormalizedDictionary`，其数据布局与普通标准字典一致。运行时只消费合并后的结果，而不反复执行 overlay 逻辑。
...
- baseline typed view 按标准字典生成。
- overlay typed view 可按具体对手方选择性生成。
- generic API 始终可用，用于访问未知或临时扩展字段。
- session 代码依赖 `TransportSessionProfile` 与 `ValidationPolicy`，而不是依赖单一 `FIXVersion` 枚举。

这样可以让“支持新版本”和“支持新方言”成为加 profile 的动作，而不是改 parser 核心。
...
- 标准 FIX XML 作为 baseline。
- venue / broker / counterparty 的 overlay 作为增量定义。
- validation policy 与 profile 绑定规则作为独立配置。

2. 生成层

- baseline 与 overlay 合成为 `NormalizedDictionary`。
- 为热点消息生成 typed view、typed builder、template encoder。
- 为 generic fallback 生成紧凑元数据、查找表和规则表。
- 生成一个或多个可装载的 protocol profile artifact。
...
steady-state 路径不再解析 XML，不再合并 overlay，也不再临时构造大对象图。
...
- 带有 `magic`、format version、schema hash、profile id、build id。
- 支持只读映射和零拷贝视图封装。
...
session 建立时只保存对已装载 profile 的只读引用。steady-state 下不再触碰 XML、overlay 合并器或大规模初始化逻辑。

生成代码与 artifact 之间必须携带同一个 `schema_hash` 或 `build_id`，启动时强制校验一致性，避免代码与元数据错配。

### 6.15 Hot message 策略

真实世界里高频路径上最常见的不是“纯标准 FIX 消息”，而是“某个 baseline 版本上叠加一组稳定 custom 字段”的 profile 消息。

因此 hot path 的建模单位不应是“FIX 4.4 消息”，而应是“某个 counterparty profile 下的消息模板”。
...
- 直接纳入 `DialectOverlay`。
- 进入 `NormalizedDictionary`。
- 为其生成 profile-specific typed view、typed builder 和 template encoder。

2. 低频但已知的扩展字段

- 保留在 profile 中。
- 允许使用 typed builder + extension setter。

3. 临时、调试或未知字段

- 走 generic tag writer / generic field lookup。
- 不作为 hot path 主要接口。

因此发送路径建议分层为：

- profile-specific generated builder + template encoder：默认 hot path
- typed builder + 少量 extension setter：次优路径
- generic tag writer：冷路径与兜底路径
...
- 面向热点业务消息。
- 预先生成固定片段，仅回填变动字段、`BodyLength`、`CheckSum`。
...
- `ResolvedProtocolProfile` 用于描述该会话绑定的应用字典、方言和校验策略。
...
- 支持在 Logon 后将连接绑定到 `CounterpartyProfile`，并据此选择 transport profile、application baseline 与 dialect overlay。
- 支持未知 session 的拒绝策略。
...
1. 先定义 `TransportSessionProfile`、`ApplicationDictionaryBaseline`、`DialectOverlay`、`ValidationPolicy` 的数据模型与合并规则。
2. 再定义字典生成器输出格式与 `NormalizedDictionary` 布局。
