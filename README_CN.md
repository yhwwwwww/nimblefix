...
| **FIX 官方 session 测试案例** | **部分覆盖** | 已接入离线 FIX Trading session-case manifest 和可执行 `.nfscenario` baseline；当前 85 个官方 case 中已有 73 个映射到仓内通过的场景，剩余 12 个 optional case 仍以 unsupported 形式显式跟踪 |
...
| 工具 | 用途 |
|------|------|
| `nimblefix-dictgen` | 将 `.nfd` 字典编译为二进制 `.nfa` profile（支持合并多个 `.nfd` 文件，可生成 C++ builder 头文件） |
| `nimblefix-xml2nfd` | 将 QuickFIX XML 数据字典转换为 `.nfd` 格式；也可从 `.nfd` 生成 C++ builder 头文件 |
...
```bash
./build/linux/x86_64/release/nimblefix-dictgen     --input samples/basic_profile.nfd     --merge samples/basic_overlay.nfd     --output build/sample-basic.nfa     --cpp-builders build/generated/sample_basic_builders.h
```
...
```bash
./build/linux/x86_64/release/nimblefix-xml2nfd     --xml FIX44.xml     --output my_profile.nfd     --profile-id 1001     --cpp-builders generated_builders.h
```
