...
| 特性 | 说明 |
|------|------|
| **编解码延迟** | FIX44 benchmark：header peek ~198 ns，完整解析 ~1.3 µs，fixed-layout 编码 ~684 ns |
| **稳态零分配** | encode-buffer 路径：预热后每条消息 0 次堆分配 |
...
# 构建特定目标
xmake build fastfix           # 静态库
xmake build fastfix-bench     # 性能测试
xmake build fastfix-tests     # 单元测试
xmake build fastfix-dictgen   # 字典编译器
xmake build fastfix-xml2ffd   # QuickFIX XML → .ffd 转换器
xmake build fastfix-initiator # CLI 客户端
xmake build fastfix-acceptor  # CLI 服务端
```

在 Linux x86_64 的 release 构建下，二进制位于 `./build/linux/x86_64/release/`。下面的示例默认直接执行二进制；`xmake run <target>` 保留为便捷替代方式。

### 运行测试

```bash
./build/linux/x86_64/release/fastfix-tests
# 或: xmake run fastfix-tests
...
1. **编译库**：`xmake build fastfix` — 生成 `build/linux/x86_64/release/libfastfix.a`
...
#    链接库:       path/to/fastfix/build/linux/x86_64/release/libfastfix.a
#    C++ 标准:     C++20
```
...
| **encode** | 从 `Message` 对象构建 FIX 帧（每次新分配缓冲区） |
| **encode-buffer** | 同上但复用预分配缓冲区 |
| **builder-generic-e2e-buffer** | 通用 `MessageBuilder` → 报文（固定 SendingTime，复用缓冲区） |
| **builder-fixed-layout-buffer** | `FixedLayoutWriter` → 报文（固定 SendingTime，复用缓冲区） |
| **builder-fixed-layout-hybrid-buffer** | `FixedLayoutWriter` + 扩展字段 → 报文 |
...
| 边界 | FastFix 指标 | QuickFIX 指标 | FastFix p50 | QuickFIX p50 | FastFix p99 | QuickFIX p99 | FF 每次分配 | QF 每次分配 |
|------|--------------|---------------|-------------|--------------|-------------|--------------|-------------|-------------|
...
- **FastFix 的 fixed-layout 与 hybrid order-to-wire 路径已快于 QuickFIX 的 buffer serializer**，分别约快 46% 和 40%。
- **所有可比层级上，FastFix 的分配压力更低**：parse 为 2/msg vs 15/msg；object-to-wire 为 9-15/op vs 29/op。

#### FastFix 专有层级

| 层级 | p50 | p99 | 每次分配 | 吞吐量 |
|------|-----|-----|----------|--------|
| peek | 180 ns | 191 ns | 0 | 5,043,875/s |
| session-inbound | 2.77 µs | 4.69 µs | 2 | 335,507/s |
| replay（128 msgs） | 532 µs | 623 µs | 257 | 1,831/s |
| loopback-roundtrip | 25.66 µs | 32.27 µs | 7 | 37,755/s |
