...
| **高可用** | Active/standby HA，支持可配置的故障转移、心跳监控和状态复制 |
| **动态配置** | 通过 `ApplyConfig()` 热重载 counterparty、listener 和引擎参数，无需完整重启 |
| **预热** | 启动时预热编解码和 profile 路径，降低首条消息延迟抖动 |
| **诊断** | 通过 `DiagnosticsMonitor` 获取引擎健康快照，支持可插拔 sink（JSON、文本） |
| **管理平面** | 运行时查询引擎/会话状态、强制断连、触发日切、重置序列号 |
...
高级用法再按需补 `nimblefix/advanced/runtime_application.h`、`nimblefix/advanced/engine.h`、`nimblefix/advanced/message_builder.h`、`nimblefix/advanced/fixed_layout_writer.h`、`nimblefix/advanced/encoded_application_message.h`、`nimblefix/advanced/session_handle.h`、`nimblefix/session/admin_protocol.h`、`nimblefix/session/resend_recovery.h`、`nimblefix/runtime/sharded_runtime.h`、`nimblefix/runtime/metrics.h`、`nimblefix/runtime/trace.h`、`nimblefix/runtime/ha.h`、`nimblefix/runtime/dynamic_config.h`、`nimblefix/runtime/warmup.h`、`nimblefix/runtime/diagnostics.h`、`nimblefix/runtime/management.h`、`nimblefix/runtime/message_log.h`、`nimblefix/runtime/connection_strategy.h`、`nimblefix/runtime/session_schedule.h`、`nimblefix/runtime/router.h`、`nimblefix/runtime/io_backend.h`、`nimblefix/session/validation_callback.h`、`nimblefix/codec/timestamp_resolution.h`、`nimblefix/tools/schema_optimizer.h`、`nimblefix/tools/message_dump.h`。完整的导出头策略见 [docs/public-api.md](docs/public-api.md)。
...
- **手工 fixed-layout 编码控制** — 只有在你明确要自己管理 dictionary/layout 细节时才使用，放在 `nimblefix/advanced/` 下。
...
| `nimblefix-fuzz-config` | libFuzzer，覆盖 `.nfcfg` 配置解析 |
| `nimblefix-fuzz-dictgen` | libFuzzer，覆盖 `.nfd` 字典解析 |
...
| `backlog_warn_threshold_ms` | uint32 | 5000 | 当会话入站积压超过此阈值（ms）时记录告警 |
| `backlog_warn_throttle_ms` | uint32 | 1000 | 同一会话积压告警的最小间隔（ms） |
...
| `warmup_message_count` | uint32 | 0 | 会话启动时预编码的预热消息数 |
...
| 应用 worker | `nf-app-w{N}` | `worker_count` | 消费 SPSC 队列，调用业务回调 | `app_cpu_affinity[N]` |

**Initiator（客户端）：** 与 acceptor 相同但没有前门线程。会话 worker 命名为 `nf-ini-w{N}`。
...
NimbleFIX 正在积极开发中。核心引擎、会话管理、编解码器和持久化层已实现并测试（403 test case，5200+ assertion）。热路径编码管线持续向设计目标优化中。
