...
3. **链接和引用头文件**：编译器指向 `include/` 目录，链接 `libnimblefix.a`
...
target("my-trading-app")
    set_kind("binary")
    add_deps("nimblefix")        -- 自动链接 libnimblefix.a + 添加 include 路径
    add_files("src/*.cpp")
```
...
# 2. 在你的构建系统中添加：
#    头文件路径:    path/to/nimblefix/include
...
跨线程访问 session 仍然通过 `SessionHandle`，但 `Send*()` / `SendEncoded*()` 背后是每个 worker 一条 SPSC 命令队列。`Snapshot()` 和 `Subscribe()` 可以安全地从任意线程调用。发送路径严格要求单生产者：runtime 会把每个 handle 的发送队列绑定到第一次调用它的线程，如果随后有其他线程也向同一个 session 发送，会直接返回 `kInvalidArgument`，而不是静默破坏 SPSC 队列语义。
