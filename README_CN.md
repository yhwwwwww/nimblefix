...
GitHub Actions CI 会在 Ubuntu 上使用默认 xmake 路径，同时继续通过 `ubi8/ubi:8.10` + `gcc-toolset-12` 和 `ubi9/ubi:9.7` + `gcc-toolset-14` 容器 job 实际跑这两个 RHEL CMake preset。
...
# 或：CMake 备用二进制: ./build/cmake/dev-release/bin/fastfix-tests
# 或：make 兜底二进制: ./build/cmake/dev-release-make/bin/fastfix-tests
```
...
1. **编译库**：`xmake f -m release -y && xmake build fastfix`；如果需要备用路径，再用 `cmake --build build/cmake/dev-release --target fastfix`，而强制 make 兜底时对应目录是 `build/cmake/dev-release-make`
