...
| 特性 | 说明 |
|------|------|
| **编解码延迟** | 当前 FIX44 对比实测：header peek 130 ns (p50)，完整解析 511 ns (p50) |
| **稳态零分配** | 当前 compare 实测：parse/inbound/replay 全部 0 alloc/op；loopback 为 3 alloc/op |
...
|------|---------------|--------------|-------------|-------------|-------------|-------------|-------------------|-------------------|
| 用户编码 | `encode` | `quickfix-encode` | — | — | — | — | — | — |
| 会话出站 | `outbound` | `quickfix-outbound` | — | — | — | — | — | — |
| 线格式 → 对象 | `parse` | `quickfix-parse` | 511 ns | 521 ns | 1.29 us | 1.33 us | 0 | 20 |
| 会话入站 | `inbound` | `quickfix-inbound` | 1.65 us | 1.94 us | 2.38 us | 2.75 us | 0 | 18 |
| 重放 (`replay_span=128`) | `replay` | `quickfix-replay` | 15.66 us | 16.81 us | 231.20 us | 269.07 us | 0 | 4117 |
| TCP 回环往返 | `loopback` | `quickfix-loopback` | 17.58 us | 20.75 us | 20.55 us | 24.68 us | 3 | 77 |
...
- 当前仓库内的 side-by-side 实测里，NimbleFIX 在所有共享边界上都领先：parse 约 2.5x、inbound 约 1.4x、replay 约 14.8x、loopback RTT 约 1.2x。
