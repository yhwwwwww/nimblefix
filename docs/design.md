...
- 若需要在标准字典上叠加对手方/交易所的扩展字段，由离线字典生成工具（`fastfix-dictgen`）完成合并，运行时只消费合并后的结果。
...
字典合并仅发生在离线字典生成工具（`fastfix-dictgen`）中，运行时不存在合并逻辑。
...
发送时所有需要发送的字段必须在 `NormalizedDictionary` 中声明。发送路径分为三层：

1. Hot path：profile-specific generated builder + template encoder

- 面向高频业务消息。
- 所有字段（包括 venue 自定义字段）通过字典声明，由 `FixedLayout` 分配 O(1) slot。
- 编码使用预编译模板，仅回填变动字段。

2. Generic path：`MessageBuilder` + generic encoder

- 面向低频、调试或临时场景。
- 按 tag 逐字段写入，不依赖预编译 layout。
- 不做字典校验，适用于开发期和诊断工具。

3. Forwarding path：header rewrite + raw body passthrough
...
不提供介于 hot path 和 generic path 之间的”extension setter”路径。若一个字段需要在生产路径发送，它就应该在字典中。
