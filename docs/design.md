...
- 支持明文 TCP 与可选 TLS transport，且 TLS 不改变 codec、session 与 store 的协议语义边界。
...
- TLS 作为可选 transport 能力提供；核心 codec/session 内核不依赖 OpenSSL 或 TLS 状态，也不把 TLS 身份混入 FIX `SessionKey`。
- 不以 Web 管理界面、复杂插件系统为目标。
- 不以内核旁路网络为前提条件。
- 不要求把所有消息都建模成强类型对象树。
...
- 封装 socket I/O、连接建立、监听、accept、reconnect 和可选 TLS 握手。
- 为 session core 提供统一 transport 接口。
- 对上暴露完整 FIX frame 的收发语义；明文 TCP 与 TLS 只在字节流加解密层不同，不改变 frame 边界识别、session 序号或重发语义。
...
3. Generic path：advanced/tooling `MessageDataWriter` + generic encoder
...
### 13.4 TLS transport 设计

TLS 是 `nimblefix-transport` 的可选传输安全层，而不是 FIX 协议身份的一部分。明文 TCP 与 TLS 连接在 session core 看来都应是同一种 `TransportConnection` 能力集合：暴露 `fd()`、`Send()`、`SendGather()`、`TryReceiveFrameView()`、`ReceiveFrameView()`、`Close()` 等 frame 级接口。codec 仍只处理解密后的 FIX 字节流，session 仍只处理 Logon、序号、心跳、重发和 store 语义。

推荐引入一个值语义连接封装：

```text
TransportConnection {
    kind = PlainTcp | TlsClient | TlsServer
    fd()
    Send(frame)
    SendGather(frame_segments)
    TryReceiveFrameView()
    Close()
}
```

内部可以用 `std::variant<TcpConnection, TlsConnection>` 或等价的无堆分配类型擦除实现。目标是让 runtime 的 `ConnectionState` 只持有 `TransportConnection`，而不在 initiator、acceptor、worker、poller 和发送批处理路径上散落 `if tls` 分支。公共热路径允许一次按连接类型分派，但不应引入每字段、每 tag 或每 session 状态机步骤的 TLS 分支。

`TlsConnection` 复用现有 TCP fd、poller 和 FIX frame 抽取逻辑，差异只在读写字节流时调用 TLS record 层：

- 握手使用非阻塞状态机处理 `WANT_READ` / `WANT_WRITE`，受现有 connect / I/O timeout 约束。
- 接收侧先把 TLS record 解密到连接私有的预分配 read buffer，再使用与 TCP 相同的 FIX frame 边界检测。
- `SSL_pending` 中已有解密字节时，即使 fd 暂无新的 readiness，也应继续尝试抽取完整 frame。
- 发送侧可以逐段写 TLS record；`SendGather()` 在 TLS 下保持有序 segment 语义，但不承诺内核 zero-copy。
- `SendZeroCopyGather()` 在 TLS 连接上降级为普通 TLS gather send，因为加密必然需要用户态处理或 TLS offload 的单独能力协商。

TLS 握手发生在 FIX Logon 之前。Initiator 的顺序为 TCP connect -> 当 `TlsClientConfig.enabled=true` 时执行 TLS client handshake -> `OnTransportConnected()` -> 发送 Logon。Acceptor 的顺序为 TCP accept -> 当 `ListenerConfig.tls_server.enabled=true` 时执行 TLS server handshake -> 握手完成后才读取 Logon 并绑定 `FixSession`。若握手失败、证书校验失败或超时，连接直接关闭，不进入 session 状态机，也不消耗业务序号。`NIMBLEFIX_ENABLE_TLS` 只表示二进制是否编译进 TLS support / 链接 OpenSSL；运行时是否加密只看 `TlsClientConfig.enabled` / `TlsServerConfig.enabled`，默认仍是明文 TCP。
...
TlsClientConfig {
    ...
    expected_peer_name
    ca_file / ca_path
    certificate_chain_file / private_key_file   # 可选，用于 mTLS
    verify_peer = true
    min_version / max_version
    cipher_list / cipher_suites
    session_resumption
}
...
- Initiator 从 `CounterpartyConfig` 取得 `TlsClientConfig`。
- Acceptor 从 `ListenerConfig` 取得 `TlsServerConfig`，因为是否加密首先是监听端口属性。
- `CounterpartyConfig` 可增加 accept-side transport security requirement（例如 `any`、`plain-only`、`tls-only`），避免敏感 session 被明文 listener 绑定。
- TLS peer identity 可作为认证和准入策略输入，但不参与 `SessionKey`、worker hash、序号 store key 或 profile id。

配置验证必须在 `ValidateEngineConfig()` 阶段完成基本约束检查：当 `TlsClientConfig.enabled=true` 或 `TlsServerConfig.enabled=true` 时，证书和私钥路径必须成对存在，server 证书为 acceptor TLS 必填，CA/hostname 校验默认开启，`verify_peer=false` 应显式配置并可被文档标注为测试或受控内网模式。构建系统应提供可选 OpenSSL 依赖开关；`NIMBLEFIX_ENABLE_TLS` 只控制是否编译进 optional TLS support，未编译 support 时任何 `enabled=true` 的 TLS 运行时配置都必须启动失败，而不是静默降级成明文。

TLS 上下文生命周期应按角色复用：client context 可按相同信任配置缓存，server context 按 listener 缓存。握手对象属于单个 `TransportConnection`，在连接关闭时释放。证书、私钥和 CA 文件只在配置加载或 listener/open-session 冷路径读取，不进入 message steady-state 路径。

可观测性需至少覆盖：TLS 握手成功/失败次数、失败原因分类、握手耗时、协商协议版本、cipher 名称、是否使用 session resumption。trace 不应记录私钥、证书完整内容或明文凭据；如需排障，只记录证书 subject、issuer、SAN 摘要或 fingerprint。
...
- TLS 连接复用同一 poller 与 frame 抽取接口；明文路径不承担 TLS 状态开销，TLS 路径不承诺 zero-copy send。
- 支持预热对象池与消息模板。
...
- 标准管理消息往返测试
- 不同 FIX 版本字典校验测试
- TLS initiator / acceptor 与常见 OpenSSL client/server 工具互通测试
...
- TLS 握手超时、证书链错误、主机名不匹配、mTLS 客户端证书缺失、协议版本或 cipher 不匹配
...
- 分别覆盖明文 TCP 与 TLS steady-state，TLS 单独报告握手耗时、session resumption 命中率和加密吞吐。
...
- reconnect 退避
- socket 选项
- TLS client/server 配置、证书链、私钥、CA、hostname 校验、mTLS、协议版本、cipher 与 session resumption
