...
void
SetEncodeStepPrefix(FixedLayout::EncodeStep& step, std::uint32_t tag)
{
...
void
AppendTagToBuffer(std::string& buf, std::uint32_t tag)
{
  char tmp[10];
...
auto
FixedLayout::Build(const profile::NormalizedDictionaryView& dictionary, std::string_view msg_type)
  -> base::Result<FixedLayout>
{
  ...
  std::sort(
    layout.field_slots_.begin(), layout.field_slots_.end(), [](const auto& a, const auto& b) { return a.tag < b.tag; });
  std::sort(layout.group_slots_.begin(), layout.group_slots_.end(), [](const auto& a, const auto& b) {
    return a.count_tag < b.count_tag;
  });
...
auto
FixedLayout::slot_index(std::uint32_t tag) const -> int
{
...
auto
FixedLayout::group_slot_index(std::uint32_t count_tag) const -> int
{
  auto it =
    std::lower_bound(group_slots_.begin(), group_slots_.end(), count_tag, [](const GroupSlot& slot, std::uint32_t t) {
      return slot.count_tag < t;
    });
...
auto
FixedGroupEntryBuilder::set_string(std::uint32_t tag, std::string_view value) -> FixedGroupEntryBuilder&
{
...
auto
FixedGroupEntryBuilder::set_int(std::uint32_t tag, std::int64_t value) -> FixedGroupEntryBuilder&
{
  AppendTagToBuffer(*buffer_, tag);
  buffer_->push_back('=');
  char buf[20];
...
auto
FixedGroupEntryBuilder::set_char(std::uint32_t tag, char value) -> FixedGroupEntryBuilder&
{
  AppendTagToBuffer(*buffer_, tag);
  buffer_->push_back('=');
...
auto
FixedGroupEntryBuilder::set_float(std::uint32_t tag, double value) -> FixedGroupEntryBuilder&
{
  if (buffer_ == nullptr || !std::isfinite(value)) {
    return *this;
  }

  // On conversion failure we omit the field entirely rather than leaving a
  // partial tag=value prefix.
...
auto
FixedGroupEntryBuilder::set_boolean(std::uint32_t tag, bool value) -> FixedGroupEntryBuilder&
{
  AppendTagToBuffer(*buffer_, tag);
  buffer_->push_back('=');
...
auto
FixedLayoutWriter::bind_session(std::string_view begin_string,
                                ...
                                std::string_view target_comp_id) -> void
{
  ...
  frag.header_prefix.reserve(2U + begin_string.size() + 1U + // "8={bs}\x01"
...
auto
FixedLayoutWriter::set_string(std::uint32_t tag, std::string_view value) -> FixedLayoutWriter&
{
  const auto idx = layout_->slot_index(tag);
  if (idx >= 0) {
    const auto slot = static_cast<std::size_t>(idx);
    slot_ranges_[slot] =
      SlotRange{ static_cast<std::uint32_t>(slot_buffer_.size()), static_cast<std::uint32_t>(value.size()) };
...
auto
FixedLayoutWriter::set_int(std::uint32_t tag, std::int64_t value) -> FixedLayoutWriter&
{
  const auto idx = layout_->slot_index(tag);
  if (idx >= 0) {
    const auto slot = static_cast<std::size_t>(idx);
    char buf[20];
    const auto len = codec::FormatInt64(buf, value);
    slot_ranges_[slot] = SlotRange{ static_cast<std::uint32_t>(slot_buffer_.size()), static_cast<std::uint32_t>(len) };
...
auto
FixedLayoutWriter::set_char(std::uint32_t tag, char value) -> FixedLayoutWriter&
{
  const auto idx = layout_->slot_index(tag);
  if (idx >= 0) {
    const auto slot = static_cast<std::size_t>(idx);
    slot_ranges_[slot] = SlotRange{ static_cast<std::uint32_t>(slot_buffer_.size()), 1 };
...
auto
FixedLayoutWriter::set_float(std::uint32_t tag, double value) -> FixedLayoutWriter&
{
  const auto idx = layout_->slot_index(tag);
  if (idx >= 0) {
    const auto slot = static_cast<std::size_t>(idx);
    ...
    if (ec == std::errc()) {
      ...
      slot_ranges_[slot] =
        SlotRange{ static_cast<std::uint32_t>(slot_buffer_.size()), static_cast<std::uint32_t>(len) };
...
auto
FixedLayoutWriter::set_boolean(std::uint32_t tag, bool value) -> FixedLayoutWriter&
{
  const auto idx = layout_->slot_index(tag);
  if (idx >= 0) {
    const auto slot = static_cast<std::size_t>(idx);
...
auto
FixedLayoutWriter::add_group_entry(std::uint32_t count_tag) -> FixedGroupEntryBuilder
{
  for (auto& g : groups_) {
...
auto
FixedLayoutWriter::reserve_group_entries(std::uint32_t count_tag, std::size_t count) -> void
{
  for (auto& g : groups_) {
    if (g.count_tag == count_tag) {
...
auto
FixedLayoutWriter::encode_to_buffer(const profile::NormalizedDictionaryView& /*dictionary*/,
                                    ...
                                    codec::EncodeBuffer* buffer) const -> base::Status
{
  ...
  if (session_bound_ && delimiter == codec::kFixSoh) {
    ...
    // 1-3. Header prefix (BeginString + BodyLength placeholder + MsgType) —
    // memcpy
    ...
    if (step.kind == FixedLayout::EncodeStep::Kind::kField) {
      ...
      for (const auto& g : groups_) {
        if (g.count_tag == step.tag) {
          ...
          out.append(step.prefix());
          char count_buf[10];
          const auto count_len = codec::FormatUint32(count_buf, static_cast<std::uint32_t>(g.active_count));
...
auto
FixedLayoutWriter::encode(const profile::NormalizedDictionaryView& dictionary,
