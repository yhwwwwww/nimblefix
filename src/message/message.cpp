...
auto FindField(std::vector<FieldValue>& fields, std::uint32_t tag) -> FieldValue* {
...
auto FindField(const std::vector<FieldValue>& fields, std::uint32_t tag) -> const FieldValue* {
...
auto FindGroup(std::vector<GroupData>& groups, std::uint32_t count_tag) -> GroupData* {
...
auto FindGroup(const std::vector<GroupData>& groups, std::uint32_t count_tag) -> const GroupData* {
...
auto UpsertField(std::vector<FieldValue>& fields, FieldValue value) -> void {
...
auto ParsedSlotText(const ParsedMessageData& data, const ParsedFieldSlot& slot) -> std::string_view {
...
auto NthParsedGroup(const ParsedMessageData& data, const ParsedEntryData& entry, std::size_t index)
    -> const ParsedGroupFrame* {
    auto group_index = entry.first_group;
...
auto NthParsedEntry(const ParsedMessageData& data, const ParsedGroupFrame& group, std::size_t index)
...
auto CopyViewToOwned(MessageView view) -> MessageData {
    MessageData data;
...
auto RebindParsedMessageDataRaw(ParsedMessageData* data, std::span<const std::byte> raw) -> void {
    if (data == nullptr) {
        return;
    }
...
auto MessageBuilder::view() const -> MessageView {
    return MessageView(&data_);
}

auto MessageBuilder::encode_to_buffer(
...
auto MessageBuilder::upsert_field(FieldValue value) -> MessageBuilder& {
    UpsertField(data_.fields, std::move(value));
    return *this;
}
...
auto MessageBuilder::reserve_groups(std::size_t count) -> MessageBuilder& {
    data_.groups.reserve(count);
    return *this;
}

auto MessageBuilder::reserve_group_entries(std::uint32_t count_tag, std::size_t count) -> MessageBuilder& {
    ensure_group(count_tag).entries.reserve(count);
    return *this;
}

auto MessageBuilder::ensure_group(std::uint32_t count_tag) -> GroupData& {
    if (auto* group = FindGroup(data_.groups, count_tag); group != nullptr) {
        return *group;
    }
...
auto MessageBuilder::reset() -> void {
    data_.fields.clear();
...
auto RawGroupEntryView::field_count() const -> std::size_t {
    return parsed_entry_ == nullptr ? 0U : parsed_entry_->field_count;
}

auto RawGroupEntryView::field_at(std::size_t index) const -> std::optional<RawFieldView> {
...
auto RawGroupEntryView::field(std::uint32_t tag) const -> std::optional<std::string_view> {
    if (parsed_ == nullptr || parsed_entry_ == nullptr) {
        return std::nullopt;
    }
...
auto RawGroupEntryView::group(std::uint32_t count_tag) const -> std::optional<RawGroupView> {
    if (parsed_ == nullptr || parsed_entry_ == nullptr) {
        return std::nullopt;
    }
...
auto RawGroupView::Iterator::operator*() const -> RawGroupEntryView {
...
auto RawGroupView::operator[](std::size_t index) const -> RawGroupEntryView {
...
auto ParsedMessage::view() const -> MessageView {
    return MessageView(&data_, data_.msg_type, &data_.root);
}

auto ParsedMessage::RebindRaw(std::span<const std::byte> raw) -> void {
    RebindParsedMessageDataRaw(&data_, raw);
}

auto ParsedMessage::ToOwned() const -> Message {
...
auto MessageRef::Own(MessageView view) -> MessageRef {
    if (!view.valid()) {
...
auto MessageRef::OwnParsed(ParsedMessage parsed, std::span<const std::byte> raw) -> MessageRef {
    std::vector<std::byte> owned_raw;
    owned_raw.assign(raw.begin(), raw.end());
    return OwnParsed(std::move(parsed), std::move(owned_raw));
}

auto MessageRef::OwnParsed(ParsedMessage parsed, std::vector<std::byte> raw) -> MessageRef {
...
auto MessageRef::operator=(const MessageRef& other) -> MessageRef& = default;

auto MessageRef::operator=(MessageRef&& other) noexcept -> MessageRef& = default;

auto MessageRef::ToOwned() const -> Message {
    if (owned_ != nullptr) {
...
auto GroupView::operator[](std::size_t index) const -> MessageView {
    if (group_ != nullptr) {
...
auto GroupView::begin() const -> Iterator {
    if (group_ != nullptr) {
...
auto GroupView::end() const -> Iterator {
    if (group_ != nullptr) {
...
auto MessageView::field_count() const -> std::size_t {
    if (data_ != nullptr) {
...
auto MessageView::field_at(std::size_t index) const -> std::optional<FieldView> {
    if (data_ != nullptr) {
...
auto MessageView::group_count() const -> std::size_t {
    if (data_ != nullptr) {
...
auto MessageView::group_at(std::size_t index) const -> std::optional<GroupView> {
    if (data_ != nullptr) {
...
auto MessageView::find_field_view(std::uint32_t tag) const -> std::optional<FieldView> {
    if (data_ != nullptr) {
...
auto MessageView::find_field(std::uint32_t tag) const -> const FieldValue* {
    if (data_ == nullptr) {
        return nullptr;
    }
...
auto MessageView::has_field(std::uint32_t tag) const -> bool {
...
auto MessageView::group(std::uint32_t count_tag) const -> std::optional<GroupView> {
    if (data_ != nullptr) {
...
auto MessageView::raw_group(std::uint32_t count_tag) const -> std::optional<RawGroupView> {
    if (parsed_ == nullptr || parsed_entry_ == nullptr) {
        return std::nullopt;
    }
