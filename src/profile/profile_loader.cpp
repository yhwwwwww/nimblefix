...
auto IoMessage(const std::filesystem::path& path, const char* action) -> std::string {
    return action + std::string(" '") + path.string() + "': " + std::strerror(errno);
}

auto InBounds(std::uint64_t offset, std::uint64_t length, std::uint64_t total) -> bool {
    if (offset > total) {
        return false;
    }
...
auto MultiplyWillOverflow(std::uint64_t lhs, std::uint64_t rhs) -> bool {
    if (lhs == 0 || rhs == 0) {
        return false;
    }
...
auto ValidateArtifact(
    ...
    if constexpr (std::endian::native != std::endian::little) {
        return base::Status::VersionMismatch(
            "artifact loading currently requires a little-endian host");
    }

    if (size < sizeof(ArtifactHeader)) {
        return base::Status::FormatError(
            "artifact '" + path.string() + "' is smaller than the header");
    }

    const auto* header = reinterpret_cast<const ArtifactHeader*>(data);
    if (!HasArtifactMagic(*header)) {
        return base::Status::FormatError(
            "artifact '" + path.string() + "' has an invalid magic");
    }

    if (header->format_version != kArtifactFormatVersion) {
        return base::Status::VersionMismatch(
            "artifact '" + path.string() + "' has an unsupported format version");
    }

    if (header->header_size != sizeof(ArtifactHeader)) {
        return base::Status::FormatError(
            "artifact '" + path.string() + "' has an unexpected header size");
    }

    if (header->section_entry_size != sizeof(ArtifactSection)) {
        return base::Status::FormatError(
            "artifact '" + path.string() + "' has an unexpected section entry size");
    }

    if (!HasLittleEndianTag(*header)) {
        return base::Status::VersionMismatch(
            "artifact '" + path.string() + "' has an unsupported endian tag");
    }

    if (header->file_size != size) {
        return base::Status::FormatError(
            "artifact '" + path.string() + "' has a mismatched file size");
    }

    const auto section_table_bytes =
        static_cast<std::uint64_t>(header->section_count) * sizeof(ArtifactSection);
    if (header->section_table_offset < header->header_size) {
        return base::Status::FormatError(
            "artifact '" + path.string() + "' has a section table overlapping the header");
    }
    if (!InBounds(header->section_table_offset, section_table_bytes, header->file_size)) {
        return base::Status::FormatError(
            "artifact '" + path.string() + "' has an out-of-bounds section table");
    }

    const auto section_table_end = header->section_table_offset + section_table_bytes;

    const auto* sections = reinterpret_cast<const ArtifactSection*>(
    ...
    for (const auto& section : sections_out) {
        if (!InBounds(section.offset, section.size, header->file_size)) {
            return base::Status::FormatError(
                "artifact '" + path.string() + "' has an out-of-bounds section");
        }

        if (section.offset < section_table_end) {
            return base::Status::FormatError(
                "artifact '" + path.string() + "' has a section overlapping the header or section table");
        }

        if (section.entry_size == 0) {
            if (section.entry_count != 0 || section.size != 0) {
                return base::Status::FormatError(
                    "artifact '" + path.string() + "' has a malformed zero-sized section entry descriptor");
            }
        } else {
            if (MultiplyWillOverflow(section.entry_count, section.entry_size)) {
                return base::Status::FormatError(
                    "artifact '" + path.string() + "' has an overflowing section size declaration");
            }

            if (section.entry_count * section.entry_size != section.size) {
                return base::Status::FormatError(
                    "artifact '" + path.string() + "' has a section whose size does not match entry_count * entry_size");
            }
        }
    ...
    for (std::size_t i = 1; i < ranges.size(); ++i) {
        if (ranges[i - 1].second > ranges[i].first) {
            return base::Status::FormatError(
                "artifact '" + path.string() + "' has overlapping sections");
        }
    }
...
auto LoadProfileArtifact(const std::filesystem::path& path,
    ...
    auto storage = std::shared_ptr<const std::byte>(
        static_cast<const std::byte*>(mapping),
        [size](const std::byte* bytes) {
            ::munmap(const_cast<std::byte*>(bytes), size);
        });
