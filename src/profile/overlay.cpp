...
auto MergeFieldRules(std::vector<FieldRule>* rules, std::span<const FieldRule> overlay_rules) -> void {
...
auto ApplyOverlay(const NormalizedDictionary& baseline, const NormalizedDictionary& overlay)
    ...
    for (const auto& overlay_field : overlay.fields) {
        auto it = std::find_if(
            merged.fields.begin(),
            merged.fields.end(),
            [&](const auto& field) { return field.tag == overlay_field.tag; });
    ...
    for (const auto& overlay_msg : overlay.messages) {
        auto it = std::find_if(
            merged.messages.begin(),
            merged.messages.end(),
            [&](const auto& message) { return message.msg_type == overlay_msg.msg_type; });
    ...
    for (const auto& overlay_group : overlay.groups) {
        auto it = std::find_if(
            merged.groups.begin(),
            merged.groups.end(),
            [&](const auto& group) { return group.count_tag == overlay_group.count_tag; });
        ...
        if (overlay_group.delimiter_tag != 0) {
            if (it->delimiter_tag != 0 && overlay_group.delimiter_tag != it->delimiter_tag &&
                ...
                return base::Status::InvalidArgument(
                    "group overlay delimiter change requires explicit allow-delimiter-override flag");
            }
