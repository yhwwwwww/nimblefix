...
auto ValidateStringOffset(
    const StringTableView& string_table,
    std::uint32_t offset,
    const char* label) -> base::Status {
...
auto NormalizedDictionaryView::FromProfile(LoadedProfile profile) -> base::Result<NormalizedDictionaryView> {
    ...
    if (const auto section = profile.fixed_section<FieldRuleRecord>(SectionKind::kMessageFieldRules); section.has_value()) {
    ...
    for (const auto& message : message_defs.entries()) {
        ...
        status = ValidateFieldRuleRange(
            message.first_field_rule,
            message.field_rule_count,
            message_field_rules.size(),
            "message definition");
        if (!status.ok()) {
            return status;
        }
    ...
    for (const auto& group : group_defs.entries()) {
        ...
        status = ValidateFieldRuleRange(
            group.first_field_rule,
            group.field_rule_count,
            group_field_rules.size(),
            "group definition");
        if (!status.ok()) {
            return status;
        }
    ...
    // Build field index sorted by tag for binary search.
    {
        ...
        std::sort(view.field_index_.begin(), view.field_index_.end(),
            [](const TagIndexEntry& a, const TagIndexEntry& b) { return a.key < b.key; });
    }
    ...
    // Build group index sorted by count_tag for binary search.
    {
        ...
        std::sort(view.group_index_.begin(), view.group_index_.end(),
            [](const TagIndexEntry& a, const TagIndexEntry& b) { return a.key < b.key; });
    }
    ...
    // Build message type index sorted by msg_type for binary search.
    {
        ...
        std::sort(view.message_index_.begin(), view.message_index_.end(),
    ...
    // Build sorted message rule tags (parallel to message_field_rules_).
    {
        ...
        for (const auto& message : view.message_defs_.entries()) {
            ...
            std::sort(base, base + message.field_rule_count,
                [](const TagRuleEntry& a, const TagRuleEntry& b) { return a.tag < b.tag; });
        }
    }
    ...
    // Build sorted group rule tags (parallel to group_field_rules_).
    {
        ...
        for (const auto& group : view.group_defs_.entries()) {
            ...
            std::sort(base, base + group.field_rule_count,
                [](const TagRuleEntry& a, const TagRuleEntry& b) { return a.tag < b.tag; });
        }
    }
...
auto NormalizedDictionaryView::find_message(std::string_view msg_type) const -> const MessageDefRecord* {
    auto it = std::lower_bound(message_index_.begin(), message_index_.end(), msg_type,
...
auto NormalizedDictionaryView::find_group(std::uint32_t count_tag) const -> const GroupDefRecord* {
    auto it = std::lower_bound(group_index_.begin(), group_index_.end(), count_tag,
        [](const TagIndexEntry& e, std::uint32_t t) { return e.key < t; });
...
auto NormalizedDictionaryView::message_rule_allows_tag(const MessageDefRecord& record, std::uint32_t tag) const -> bool {
    const auto* base = sorted_message_rules_.data() + record.first_field_rule;
    const auto* end = base + record.field_rule_count;
    auto it = std::lower_bound(base, end, tag,
        [](const TagRuleEntry& e, std::uint32_t t) { return e.tag < t; });
    return it != end && it->tag == tag;
}

auto NormalizedDictionaryView::group_rule_allows_tag(const GroupDefRecord& record, std::uint32_t tag) const -> bool {
...
auto NormalizedDictionaryView::message_rule_index(const MessageDefRecord& record, std::uint32_t tag) const -> int {
    const auto* base = sorted_message_rules_.data() + record.first_field_rule;
    const auto* end = base + record.field_rule_count;
    auto it = std::lower_bound(base, end, tag,
        [](const TagRuleEntry& e, std::uint32_t t) { return e.tag < t; });
...
auto NormalizedDictionaryView::group_rule_index(const GroupDefRecord& record, std::uint32_t tag) const -> int {
    const auto* base = sorted_group_rules_.data() + record.first_field_rule;
    const auto* end = base + record.field_rule_count;
    auto it = std::lower_bound(base, end, tag,
        [](const TagRuleEntry& e, std::uint32_t t) { return e.tag < t; });
...
auto NormalizedDictionaryView::message_field_rules(const MessageDefRecord& record) const
    -> std::span<const FieldRuleRecord> {
    return message_field_rules_.entries().subspan(record.first_field_rule, record.field_rule_count);
}

auto NormalizedDictionaryView::group_field_rules(const GroupDefRecord& record) const
    -> std::span<const FieldRuleRecord> {
