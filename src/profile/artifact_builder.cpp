...
public:
  ...
  [[nodiscard]] auto bytes() const -> const std::vector<std::byte>& { return bytes_; }
...
auto
SerializeEntries(std::span<const Entry> entries) -> std::vector<std::byte>
{
...
auto
ValidateDictionary(const NormalizedDictionary& dictionary) -> base::Status
{
...
auto
ComputeRequiredFieldBitmap(std::span<const FieldRule> rules) -> std::pair<std::uint64_t, bool>
{
  std::uint64_t bitmap = 0U;
  bool overflow = false;
  for (std::size_t index = 0; index < rules.size(); ++index) {
    const bool required = (rules[index].flags & static_cast<std::uint32_t>(FieldRuleFlags::kRequired)) != 0U;
    if (!required) {
      continue;
    }
    if (index >= 64U) {
...
BuildProfileArtifact(const NormalizedDictionary& dictionary) -> base::Result<std::vector<std::byte>>
{
  ...
  for (const auto& group : dictionary.groups) {
    ...
    const auto [required_field_bitmap, required_bitmap_overflow] = ComputeRequiredFieldBitmap(group.field_rules);
    ...
    group_defs.push_back(GroupDefRecord{
      ...
      .flags =
        group.flags | (required_bitmap_overflow ? static_cast<std::uint32_t>(GroupFlags::kRequiredBitmapOverflow) : 0U),
  ...
  std::uint64_t offset =
    sizeof(ArtifactHeader) + static_cast<std::uint64_t>(pending_sections.size()) * sizeof(ArtifactSection);
