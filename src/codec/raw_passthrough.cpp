...
// Standard header tags that we extract in raw-pass-through mode.
auto IsSessionHeaderTag(std::uint32_t tag) -> bool {
    switch (tag) {
        case 8U: case 9U: case 35U: case 34U: case 49U: case 56U: case 52U:
            return true;
        default:
            return false;
    }
...
auto DecodeRawPassThrough(
    ...
    char delimiter) -> base::Result<RawPassThroughView> {
    ...
    while (pos < data.size()) {
        ...
        switch (f.tag) {
            ...
            case 52U:
                ...
                break;
            default:
                // First non-header field means all subsequent fields up to
                // tag 10 are body. But we keep scanning to find tag 10.
                // Only advance last_header_end for actual header tags.
                break;
        }
        pos = f.next_pos;
    }
