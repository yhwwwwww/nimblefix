...
auto CompiledMessageDecoder::Build(
    ...
    for (const auto& rule : rules) {
        ...
        // Skip tags that are handled by the header-extraction path in the decode loop.
        ...
        if (field_def != nullptr) {
            slot.field_type = FieldTypeFromValueType(
                static_cast<profile::ValueType>(field_def->value_type));
        }
...
auto CompiledDecoderTable::Build(const profile::NormalizedDictionaryView& dictionary) -> CompiledDecoderTable {
    ...
    std::sort(table.entries_.begin(), table.entries_.end(),
        [](const Entry& a, const Entry& b) { return a.msg_type < b.msg_type; });

    return table;
}

auto CompiledDecoderTable::find(std::string_view msg_type) const -> const CompiledMessageDecoder* {
    auto it = std::lower_bound(entries_.begin(), entries_.end(), msg_type,
        [](const Entry& e, std::string_view t) { return e.msg_type < t; });
