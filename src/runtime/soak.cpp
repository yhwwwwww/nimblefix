...
auto AdvanceTime(SoakSessionHarness* harness, std::uint64_t delta_ns) -> std::uint64_t {
...
auto DriveTimerPulse(
    ...
    const auto timer_ns = AdvanceTime(
        harness,
        std::max<std::uint64_t>(1U, config.timer_step_seconds) * kNanosPerSecond);
    ...
    for (const auto& test_request_id : test_request_ids) {
        auto inbound = EncodeInboundFrame(
            BuildHeartbeatMessage(test_request_id),
            dictionary,
...
auto ReconnectSession(
    ...
    auto inbound_logon = EncodeInboundFrame(
        BuildLogonMessage(harness->counterparty.session.heartbeat_interval_seconds),
        dictionary,
        harness->counterparty,
        harness->protocol->session().Snapshot().next_in_seq);
    if (!inbound_logon.ok()) {
...
auto SendOutboundApplication(
    SoakSessionHarness* harness,
    std::uint32_t round,
    const profile::NormalizedDictionaryView& dictionary,
    MetricsRegistry* metrics) -> base::Status {
    const auto outbound = harness->protocol->SendApplication(
        BuildApplicationMessage(dictionary, round, (round % 4U) == 0U).view(),
        AdvanceTime(harness, 1U));
    if (!outbound.ok()) {
...
auto ReplayOutboundRange(
    ...
    auto replay_request = EncodeInboundFrame(
        BuildResendRequestMessage(1U, 2U),
        dictionary,
        harness->counterparty,
        snapshot.next_in_seq);
    if (!replay_request.ok()) {
        return replay_request.status();
    }

    auto status = DeliverInbound(
        harness,
        std::move(replay_request).value(),
        true,
        false,
        metrics,
        dictionary,
...
auto InjectNetworkJitter(
    SoakSessionHarness* harness,
    const SoakConfig& config,
    SoakReport* report) -> void {
    ...
    AdvanceTime(
        harness,
        static_cast<std::uint64_t>(config.jitter_step_millis) * 1'000'000ULL);
    ++report->total_jitter_events;
}

auto InjectInboundGap(
    SoakSessionHarness* harness,
    std::uint32_t round,
    MetricsRegistry* metrics,
    const profile::NormalizedDictionaryView& dictionary,
    ...
    auto gap_fill = EncodeInboundFrame(
        BuildGapFillMessage(expected_seq + 1U),
        dictionary,
        harness->counterparty,
        expected_seq);
    if (!gap_fill.ok()) {
...
auto InjectInboundDrop(
    ...
    auto replayed_ahead = EncodeInboundFrame(
        ahead_message,
        dictionary,
        harness->counterparty,
        ahead_seq,
...
auto DeliverInboundApplication(
    SoakSessionHarness* harness,
    std::uint32_t round,
    MetricsRegistry* metrics,
    const profile::NormalizedDictionaryView& dictionary,
    SoakReport* report) -> base::Status {
    auto inbound = EncodeInboundFrame(
        BuildApplicationMessage(dictionary, round, (round % 3U) == 0U),
        dictionary,
        harness->counterparty,
        harness->protocol->session().Snapshot().next_in_seq);
    if (!inbound.ok()) {
...
auto DeliverDuplicateHeartbeat(
    SoakSessionHarness* harness,
    MetricsRegistry* metrics,
    const profile::NormalizedDictionaryView& dictionary,
    SoakReport* report) -> base::Status {
    auto duplicate = EncodeInboundFrame(
...
auto RunSoak(const SoakConfig& config) -> base::Result<SoakReport> {
    ...
    for (std::uint32_t round = 0; round < config.iterations; ++round) {
        for (auto& harness : sessions) {
            ...
            status = metrics.ObserveStoreFlushLatency(
                harness.counterparty.session.session_id,
