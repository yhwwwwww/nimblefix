...
auto ShardedRuntime::RouteSession(const session::SessionKey& key) const -> std::uint32_t {
    const auto hash = session::SessionKeyHash{}(key);
    return static_cast<std::uint32_t>(hash % shards_.size());
}

auto ShardedRuntime::RegisterSession(const session::SessionCore& session) -> base::Status {
...
auto ShardedRuntime::RegisterPendingConnection(const PendingConnection& pending) -> base::Status {
...
auto ShardedRuntime::UnregisterPendingConnection(std::uint64_t connection_id) -> base::Status {
...
auto ShardedRuntime::FindShard(std::uint32_t worker_id) const -> const WorkerShard* {
...
auto ShardedRuntime::FindSessionShard(std::uint64_t session_id) const -> const WorkerShard* {
...
auto ShardedRuntime::FindPendingConnectionShard(std::uint64_t connection_id) const
    -> const WorkerShard* {
    const auto it = pending_to_worker_.find(connection_id);
    if (it == pending_to_worker_.end()) {
        return nullptr;
