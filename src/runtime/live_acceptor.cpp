...
auto SessionKeyText(const session::SessionKey& key) -> std::string {
...
auto FrontDoorCpuAffinity(const Engine* engine) -> std::optional<std::uint32_t> {
    if (engine == nullptr || engine->config() == nullptr) {
        return std::nullopt;
    }
...
auto ApplyAcceptorFrontDoorThreadSetup(const Engine* engine, bool single_worker) -> base::Status {
    if (single_worker) {
...
auto ApplyAcceptorWorkerThreadSetup(const Engine* engine, std::uint32_t worker_id) -> base::Status {
...
auto LiveAcceptor::EnsureManagedQueueRunnerStarted() -> base::Status {
...
auto LiveAcceptor::StopManagedQueueRunner() -> base::Status {
...
auto LiveAcceptor::ResetWorkerShards(std::uint32_t worker_count) -> base::Status {
    if (worker_count == 0U) {
        worker_count = 1U;
    }
...
auto LiveAcceptor::HasActiveSession(std::uint64_t session_id) const -> bool {
    std::lock_guard lock(control_mutex_);
    return active_session_ids_.contains(session_id);
}

auto LiveAcceptor::RegisterActiveSession(std::uint64_t session_id) -> bool {
    std::lock_guard lock(control_mutex_);
    return active_session_ids_.emplace(session_id).second;
}

auto LiveAcceptor::UnregisterActiveSession(std::uint64_t session_id) -> void {
    std::lock_guard lock(control_mutex_);
    active_session_ids_.erase(session_id);
}

auto LiveAcceptor::SetTerminalStatus(base::Status status) -> void {
    std::lock_guard lock(control_mutex_);
...
auto LiveAcceptor::LoadTerminalStatus() const -> std::optional<base::Status> {
    std::lock_guard lock(control_mutex_);
    return terminal_status_;
}

auto LiveAcceptor::AdoptPendingConnections(WorkerShardState& shard) -> void {
...
auto LiveAcceptor::EnqueuePendingConnection(std::uint32_t worker_id, ConnectionState connection) -> base::Status {
...
auto LiveAcceptor::StartWorkerThreads() -> base::Status {
...
auto LiveAcceptor::StopWorkerThreads() -> void {
...
auto LiveAcceptor::WorkerLoop(std::uint32_t worker_id, std::stop_token stop_token) -> void {
    auto* shard = FindWorkerShard(worker_id);
    if (shard == nullptr) {
...
auto LiveAcceptor::SignalWorkerWakeup(std::uint32_t worker_id) -> void {
    auto* shard = FindWorkerShard(worker_id);
    if (shard == nullptr) {
        return;
    }
...
auto LiveAcceptor::ResolveWorkerId(std::uint32_t worker_id) const -> std::uint32_t {
...
auto LiveAcceptor::SelectAcceptWorkerId(std::uint32_t worker_hint) const -> std::uint32_t {
    if (worker_shards_.empty()) {
        return 0U;
    }
...
auto LiveAcceptor::FindWorkerShard(std::uint32_t worker_id) -> WorkerShardState* {
    if (worker_shards_.empty()) {
        return nullptr;
    }
...
auto LiveAcceptor::FindWorkerShard(std::uint32_t worker_id) const -> const WorkerShardState* {
    if (worker_shards_.empty()) {
        return nullptr;
    }
    return &worker_shards_[ResolveWorkerId(worker_id)];
...
auto LiveAcceptor::OpenListeners(std::string_view listener_name) -> base::Status {
    if (engine_ == nullptr || engine_->config() == nullptr) {
        return base::Status::InvalidArgument("live acceptor requires a booted engine");
    }
    if (opened_) {
...
auto LiveAcceptor::Run(std::size_t max_completed_sessions, std::chrono::milliseconds idle_timeout) -> base::Status {
    if (!opened_) {
...
auto LiveAcceptor::listener_port(std::string_view name) const -> base::Result<std::uint16_t> {
...
auto LiveAcceptor::PollOnce(std::chrono::milliseconds timeout) -> base::Status {
    if (listeners_.empty()) {
...
auto LiveAcceptor::ComputePollTimeout(std::chrono::milliseconds timeout, std::uint64_t timestamp_ns) const
...
auto LiveAcceptor::ComputePollTimeout(
...
auto LiveAcceptor::AcceptReadyListener(std::size_t listener_index, std::uint64_t timestamp_ns) -> base::Status {
    while (true) {
...
auto LiveAcceptor::ProcessConnection(
...
auto LiveAcceptor::MigrateConnectionToRoutedWorker(WorkerShardState& shard, std::size_t connection_index) -> base::Status {
    if (connection_index >= shard.connections.size()) {
...
auto LiveAcceptor::ProcessDueTimers(WorkerShardState& shard, std::uint64_t timestamp_ns) -> base::Status {
...
auto LiveAcceptor::RetryPendingAppEvent(ConnectionState& connection, std::uint64_t timestamp_ns) -> base::Status {
...
auto LiveAcceptor::BindConnectionFromLogon(
    WorkerShardState& shard,
    std::size_t connection_index,
    ...
    if (runtime != nullptr) {
        ...
        RecordTrace(
            TraceEventKind::kPendingConnectionRegistered,
            0U,
    ...
    status = DispatchSessionEvent(*bound_connection->session, SessionEventKind::kBound, timestamp_ns, "logon matched", true);
    if (!status.ok()) {
        auto* bound_shard = FindWorkerShard(bound_connection->session->worker_id);
        if (bound_shard != nullptr) {
            bound_shard->session_connection_indices.erase(bound_connection->session->counterparty.session.session_id);
...
auto LiveAcceptor::HandleInboundFrame(
    WorkerShardState& shard,
    std::size_t connection_index,
    ...
    if (!worker_threads_.empty() && connection->session != nullptr &&
        connection->session->routed_worker_id != connection_shard->worker_id &&
        !connection->close_requested && !connection->pending_app_event.has_value()) {
...
auto LiveAcceptor::HandleProtocolEvent(
    WorkerShardState& shard,
    ConnectionState& connection,
...
auto LiveAcceptor::DispatchSessionEvent(
...
auto LiveAcceptor::DispatchAdminMessage(
    const ActiveSession& session,
...
auto LiveAcceptor::DispatchAppMessage(
    ConnectionState& connection,
    message::MessageView message,
    std::uint64_t timestamp_ns) -> base::Status {
...
auto LiveAcceptor::SendFramesBatch(
    ConnectionState& connection,
    const session::ProtocolFrameList& frames,
    std::uint64_t timestamp_ns) -> base::Status {
    if (frames.size() <= 1U) {
...
auto LiveAcceptor::LoadSessionSnapshot(std::uint64_t session_id) const -> base::Result<session::SessionSnapshot> {
    std::lock_guard lock(control_mutex_);
...
auto LiveAcceptor::RegisterSessionSubscriber(std::uint64_t session_id, std::size_t queue_capacity)
...
auto LiveAcceptor::HasSessionSubscribers(std::uint64_t session_id) -> bool {
    std::lock_guard lock(control_mutex_);
...
auto LiveAcceptor::UpdateSessionSnapshot(const ActiveSession& session) -> void {
    std::lock_guard lock(control_mutex_);
    session_snapshots_[session.counterparty.session.session_id] = session.protocol->session().Snapshot();
}

auto LiveAcceptor::PublishNotification(const session::SessionNotification& notification) -> void {
...
auto LiveAcceptor::PollManagedApplicationWorker(std::uint32_t worker_id) -> base::Status {
...
auto LiveAcceptor::DrainWorkerCommands(std::uint32_t worker_id, std::uint64_t timestamp_ns) -> base::Status {
    auto* shard = FindWorkerShard(worker_id);
...
auto LiveAcceptor::RefreshConnectionTimer(WorkerShardState& shard, ConnectionState& connection, std::uint64_t timestamp_ns) -> void {
...
auto LiveAcceptor::IndexConnection(WorkerShardState& shard, std::size_t connection_index) -> void {
    if (connection_index >= shard.connections.size()) {
        return;
    }
...
auto LiveAcceptor::FindConnectionById(WorkerShardState& shard, std::uint64_t connection_id) -> ConnectionState* {
    const auto it = shard.connection_indices.find(connection_id);
...
auto LiveAcceptor::FindConnectionBySessionId(WorkerShardState& shard, std::uint64_t session_id) -> ConnectionState* {
...
auto LiveAcceptor::ReassignConnectionToWorker(
...
auto LiveAcceptor::CloseConnection(WorkerShardState& shard, std::size_t connection_index, std::uint64_t timestamp_ns) -> void {
...
auto LiveAcceptor::MarkConnectionForClose(ConnectionState& connection, std::string_view reason, bool count_completion)
    -> void {
...
auto LiveAcceptor::MakeStore(const CounterpartyConfig& counterparty) const
...
auto LiveAcceptor::MakeActiveSession(
...
auto LiveAcceptor::RecordInboundMetrics(const ActiveSession& session, std::string_view msg_type) -> void {
    const auto* config = engine_->config();
...
auto LiveAcceptor::RecordOutboundMetrics(const ActiveSession& session, const session::EncodedFrame& frame) -> void {
    const auto* config = engine_->config();
    if (config == nullptr || !config->enable_metrics) {
        return;
    }
...
auto LiveAcceptor::RecordProtocolFailure(const ActiveSession& session, const base::Status& status) -> void {
    const auto* config = engine_->config();
    if (config == nullptr || !config->enable_metrics) {
        return;
    }
