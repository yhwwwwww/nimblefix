...
auto
LiveSessionWorker::ProcessPendingReconnects(WorkerShardState& /*shard*/, std::uint64_t /*ts*/) -> base::Status
{
  return base::Status::Ok();
}

auto
LiveSessionWorker::PollWorkerOnce(WorkerShardState& shard, std::chrono::milliseconds timeout) -> base::Status
{
...
auto
LiveSessionWorker::ComputePollTimeout(const WorkerShardState& shard,
...
auto
LiveSessionWorker::ProcessDueTimers(WorkerShardState& shard, std::uint64_t timestamp_ns) -> base::Status
{
...
auto
LiveSessionWorker::RetryPendingAppEvent(ConnectionState& connection, std::uint64_t timestamp_ns) -> base::Status
{
...
auto
LiveSessionWorker::DispatchSessionEvent(const ActiveSession& session,
...
auto
LiveSessionWorker::DispatchAdminMessage(const ActiveSession& session,
...
auto
LiveSessionWorker::DispatchAppMessage(ConnectionState& connection,
                                      message::MessageView message,
                                      std::uint64_t timestamp_ns) -> base::Status
{
...
auto
LiveSessionWorker::ServiceTimer(WorkerShardState& shard, ConnectionState& connection, std::uint64_t timestamp_ns)
  -> base::Status
{
...
auto
LiveSessionWorker::SendFrame(ConnectionState& connection,
...
auto
LiveSessionWorker::SendFrames(ConnectionState& connection,
...
auto
LiveSessionWorker::SendFramesBatch(ConnectionState& connection,
                                   const session::ProtocolFrameList& frames,
                                   std::uint64_t timestamp_ns) -> base::Status
{
  if (frames.size() <= 1U) {
...
auto
LiveSessionWorker::LoadSessionSnapshot(std::uint64_t session_id) const -> base::Result<session::SessionSnapshot>
{
  return session_registry_.LoadSnapshot(session_id);
}

auto
LiveSessionWorker::RegisterSessionSubscriber(std::uint64_t session_id, std::size_t queue_capacity)
...
auto
LiveSessionWorker::HasSessionSubscribers(std::uint64_t session_id) -> bool
{
  return session_registry_.HasSubscribers(session_id);
}

auto
LiveSessionWorker::UpdateSessionSnapshot(const ActiveSession& session) -> void
{
...
auto
LiveSessionWorker::PollManagedApplicationWorker(std::uint32_t worker_id) -> base::Status
{
...
auto
LiveSessionWorker::DrainWorkerCommands(std::uint32_t worker_id, std::uint64_t timestamp_ns) -> base::Status
{
...
auto
LiveSessionWorker::RefreshConnectionTimer(WorkerShardState& shard,
...
auto
LiveSessionWorker::IndexConnection(WorkerShardState& shard, std::size_t connection_index) -> void
{
  if (connection_index >= shard.connections.size()) {
    return;
  }
...
auto
LiveSessionWorker::FindConnectionById(WorkerShardState& shard, std::uint64_t connection_id) -> ConnectionState*
{
  const auto it = shard.connection_indices.find(connection_id);
...
auto
LiveSessionWorker::FindConnectionBySessionId(WorkerShardState& shard, std::uint64_t session_id) -> ConnectionState*
{
...
auto
LiveSessionWorker::MarkConnectionForClose(ConnectionState& connection, std::string_view reason, bool count_completion)
  -> void
{
...
auto
LiveSessionWorker::EnsureManagedQueueRunnerStarted() -> base::Status
{
...
auto
LiveSessionWorker::StopManagedQueueRunner() -> base::Status
{
...
auto
LiveSessionWorker::ResetWorkerShards(std::uint32_t worker_count) -> base::Status
{
  if (worker_count == 0U) {
    worker_count = 1U;
  }
...
auto
LiveSessionWorker::StartWorkerThreads() -> base::Status
{
...
auto
LiveSessionWorker::StopWorkerThreads() -> void
{
...
auto
LiveSessionWorker::WorkerLoop(std::uint32_t worker_id, std::stop_token stop_token) -> void
{
  auto* shard = FindWorkerShard(worker_id);
  if (shard == nullptr) {
...
auto
LiveSessionWorker::AdoptPendingConnections(WorkerShardState& shard) -> void
{
...
auto
LiveSessionWorker::EnqueuePendingConnection(std::uint32_t worker_id, ConnectionState connection) -> base::Status
{
  auto* shard = FindWorkerShard(worker_id);
...
auto
LiveSessionWorker::SignalWorkerWakeup(std::uint32_t worker_id) -> void
{
  auto* shard = FindWorkerShard(worker_id);
  if (shard == nullptr) {
    return;
...
auto
LiveSessionWorker::ResolveWorkerId(std::uint32_t worker_id) const -> std::uint32_t
{
...
auto
LiveSessionWorker::FindWorkerShard(std::uint32_t worker_id) -> WorkerShardState*
{
  if (worker_shards_.empty()) {
    return nullptr;
  }
...
auto
LiveSessionWorker::FindWorkerShard(std::uint32_t worker_id) const -> const WorkerShardState*
{
  if (worker_shards_.empty()) {
    return nullptr;
  }
...
auto
LiveSessionWorker::SetTerminalStatus(base::Status status) -> void
{
  session_registry_.SetTerminalStatus(std::move(status));
}

auto
LiveSessionWorker::LoadTerminalStatus() const -> std::optional<base::Status>
{
  return session_registry_.LoadTerminalStatus();
}

auto
LiveSessionWorker::HasActiveSession(std::uint64_t session_id) const -> bool
{
  return session_registry_.HasActiveSession(session_id);
}

auto
LiveSessionWorker::RegisterActiveSession(std::uint64_t session_id) -> bool
{
  return session_registry_.RegisterActiveSession(session_id);
}

auto
LiveSessionWorker::UnregisterActiveSession(std::uint64_t session_id) -> void
{
  session_registry_.UnregisterActiveSession(session_id);
}

auto
LiveSessionWorker::MakeStore(const CounterpartyConfig& counterparty) const
...
auto
LiveSessionWorker::RecordInboundMetrics(const ActiveSession& session, std::string_view msg_type) -> void
{
...
auto
LiveSessionWorker::RecordOutboundMetrics(const ActiveSession& session, const session::EncodedFrame& frame) -> void
{
  const auto* config = engine_->config();
  if (config == nullptr || !config->enable_metrics) {
    return;
...
auto
LiveSessionWorker::RecordProtocolFailure(const ActiveSession& session, const base::Status& status) -> void
{
  const auto* config = engine_->config();
  if (config == nullptr || !config->enable_metrics) {
    return;
...
auto
LiveSessionWorker::RecordTrace(TraceEventKind kind,
...
auto
LiveSessionWorker::MakeActiveSessionBase(const CounterpartyConfig& counterparty,
