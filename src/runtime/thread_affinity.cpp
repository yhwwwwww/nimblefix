...
ApplyCurrentThreadAffinity(std::uint32_t cpu_id, std::string_view role) -> base::Status
{
  ...
  if (cpu_id >= CPU_SETSIZE) {
    return base::Status::InvalidArgument(std::string(role) + " cpu id exceeds CPU_SETSIZE");
  }
  ...
  if (rc != 0) {
    return base::Status::IoError(std::string("failed to pin ") + std::string(role) + " thread to cpu " +
                                 std::to_string(cpu_id) + ": " + std::strerror(rc));
  }
