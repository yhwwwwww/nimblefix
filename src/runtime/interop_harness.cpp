...
auto ParseBoolWord(std::string_view token) -> base::Result<bool> {
...
auto ParseSessionState(std::string_view token) -> base::Result<session::SessionState> {
    const auto value = Trim(token);
...
auto MakeStore(const CounterpartyConfig& counterparty) -> base::Result<std::unique_ptr<store::SessionStore>> {
...
auto FindMutableSession(
    ScenarioRuntimeContext& context,
    std::uint64_t session_id) -> session::SessionCore* {
...
auto FindStore(ScenarioRuntimeContext& context, std::uint64_t session_id) -> store::SessionStore* {
...
auto SaveRecoverySnapshot(
    ScenarioRuntimeContext& context,
    std::uint64_t session_id) -> base::Status {
...
auto ReplaceAndRecover(
    ScenarioRuntimeContext& context,
    std::uint64_t session_id,
    session::RecoveryMode mode) -> base::Status {
...
auto ValidateExpectations(
    ...
    const InteropReport& report) -> base::Status {
    for (const auto& expectation : scenario.session_expectations) {
        ...
        if (it->state != expectation.state || it->next_in_seq != expectation.next_in_seq ||
            it->next_out_seq != expectation.next_out_seq ||
            it->has_pending_resend != expectation.has_pending_resend) {
    ...
    for (const auto& expectation : scenario.metric_expectations) {
        ...
        if (it->inbound_messages != expectation.inbound_messages ||
            it->outbound_messages != expectation.outbound_messages ||
            it->admin_messages != expectation.admin_messages ||
            it->resend_requests != expectation.resend_requests ||
            it->gap_fills != expectation.gap_fills) {
...
auto LoadInteropScenarioText(
    std::string_view text,
    const std::filesystem::path& base_dir) -> base::Result<InteropScenario> {
    ...
    for (const auto& raw_line : SplitLines(text)) {
        ...
        if (parts[action_columns::kKind] == "action") {
            ...
            } else if (parts[action_columns::kAction] == "inbound" ||
                       parts[action_columns::kAction] == "outbound" ||
        ...
        if (parts[session_expectation_columns::kKind] == "expect-session") {
            ...
            auto has_pending = ParseInteger<std::uint32_t>(parts[session_expectation_columns::kHasPendingResend], "has_pending_resend");
            if (!session_id.ok()) {
                return session_id.status();
            }
            if (!state.ok()) {
        ...
        if (parts[metric_expectation_columns::kKind] == "expect-metric") {
            ...
            auto inbound = ParseInteger<std::uint64_t>(parts[metric_expectation_columns::kInboundMessages], "inbound_messages");
            auto outbound = ParseInteger<std::uint64_t>(parts[metric_expectation_columns::kOutboundMessages], "outbound_messages");
...
auto LoadInteropScenarioFile(const std::filesystem::path& path) -> base::Result<InteropScenario> {
...
auto RunInteropScenario(const InteropScenario& scenario) -> base::Result<InteropReport> {
    ...
    for (const auto& counterparty : scenario.engine_config.counterparties) {
        ...
        context.trace.Record(
            TraceEventKind::kSessionRegistered,
            session.session_id(),
            worker_id,
            0U,
            session.profile_id(),
    ...
    for (const auto& action : scenario.actions) {
        ...
        switch (action.kind) {
            ...
            case InteropActionKind::kOutbound: {
                ...
                status = store->SaveOutbound(MakeRecord(
                    action.session_id, seq.value(), action.is_admin, action.timestamp_ns, action.text));
                if (!status.ok()) {
                    return status;
                }
            ...
            case InteropActionKind::kInbound:
                ...
                status = store->SaveInbound(MakeRecord(
                    action.session_id, action.seq_num, action.is_admin, action.timestamp_ns, action.text));
                if (!status.ok()) {
                    return status;
                }
