...
auto
ResourceClassFor(const RuntimeEvent& event) -> QueueResourceClass
{
  return event.kind == RuntimeEventKind::kApplicationMessage ? QueueResourceClass::kApplication
                                                             : QueueResourceClass::kControl;
}

auto
ValidateWorkerLocalPoller(QueueApplication* application, QueueApplicationEventHandler* handler) -> base::Status
{
...
auto
ValidateAllWorkersPoller(QueueApplication* application, QueueApplicationEventHandler* handler) -> base::Status
{
  ...
  if (application->worker_count() > 1U) {
    return base::Status::InvalidArgument("queue application poller all-workers mode is only available for "
                                         "single-worker applications; use PollWorkerOnce/RunWorker per worker");
  }
  return base::Status::Ok();
}

auto
ApplyQueueApplicationWorkerThreadSetup(const QueueApplicationRunnerThreadOptions& options, std::uint32_t worker_id)
  -> base::Status
{
  ...
  return ApplyCurrentThreadAffinity(options.cpu_affinity[worker_id], "application worker " + std::to_string(worker_id));
}

} // namespace

auto
ApplicationCallbacks::OnSessionEvent(const RuntimeEvent& event) -> base::Status
{
  (void)event;
  return base::Status::Ok();
}
...
auto
ApplicationCallbacks::OnAdminMessage(const RuntimeEvent& event) -> base::Status
{
  (void)event;
  return base::Status::Ok();
}
...
auto
ApplicationCallbacks::OnAppMessage(const RuntimeEvent& event) -> base::Status
{
  (void)event;
  return base::Status::Ok();
}
...
struct QueueApplication::Impl
{
  ...
  {
    return options.control_queue_capacity == kUseApplicationQueueCapacity ? options.queue_capacity
  ...
  auto Push(const RuntimeEvent& event) -> base::Status
  {
    ...
    auto* queue =
      resource_class == QueueResourceClass::kControl ? control_queues[worker_id].get() : app_queues[worker_id].get();
...
auto
QueueApplication::OnSessionEvent(const RuntimeEvent& event) -> base::Status
{
  return impl_->Push(event);
}

auto
QueueApplication::OnAdminMessage(const RuntimeEvent& event) -> base::Status
{
  return impl_->Push(event);
}

auto
QueueApplication::OnAppMessage(const RuntimeEvent& event) -> base::Status
{
  return impl_->Push(event);
}

auto
QueueApplication::worker_count() const -> std::uint32_t
{
  return static_cast<std::uint32_t>(impl_->app_queues.size());
}

auto
QueueApplication::overflow_policy() const -> QueueOverflowPolicy
{
  return impl_->options.overflow_policy;
}

auto
QueueApplication::control_overflow_policy() const -> QueueOverflowPolicy
{
  return impl_->resolved_control_overflow_policy();
}

auto
QueueApplication::overflow_events() const -> std::uint64_t
{
  return impl_->overflow_events.load();
}

auto
QueueApplication::dropped_events() const -> std::uint64_t
{
  return impl_->dropped_events.load();
}

auto
QueueApplication::app_overflow_events() const -> std::uint64_t
{
  return impl_->app_overflow_events.load();
}

auto
QueueApplication::app_dropped_events() const -> std::uint64_t
{
  return impl_->app_dropped_events.load();
}

auto
QueueApplication::control_overflow_events() const -> std::uint64_t
{
  return impl_->control_overflow_events.load();
}

auto
QueueApplication::control_dropped_events() const -> std::uint64_t
{
  return impl_->control_dropped_events.load();
}

auto
QueueApplication::TryPopEvent(std::uint32_t worker_id) -> base::Result<std::optional<RuntimeEvent>>
{
...
QueueApplicationPoller::QueueApplicationPoller(QueueApplication* application,
...
auto
QueueApplicationPoller::PollWorkerOnce(std::uint32_t worker_id) -> base::Result<std::size_t>
{
  ...
  const auto max_events = options_.max_events_per_poll == kDrainAllQueueEvents ? std::numeric_limits<std::size_t>::max()
...
auto
QueueApplicationPoller::PollAllOnce() -> base::Result<std::size_t>
{
...
auto
QueueApplicationPoller::RunWorker(std::uint32_t worker_id, const std::atomic<bool>& stop_requested) -> base::Status
{
...
auto
QueueApplicationPoller::RunAll(const std::atomic<bool>& stop_requested) -> base::Status
{
  while (!stop_requested.load()) {
...
QueueApplicationRunner::QueueApplicationRunner(QueueApplication* application,
                                               std::vector<std::unique_ptr<QueueApplicationEventHandler>> handlers)
  : QueueApplicationRunner(application,
                           std::move(handlers),
...
QueueApplicationRunner::QueueApplicationRunner(QueueApplication* application,
                                               std::vector<std::unique_ptr<QueueApplicationEventHandler>> handlers,
                                               QueueApplicationPollerOptions options)
  : QueueApplicationRunner(application, std::move(handlers), options, QueueApplicationRunnerThreadOptions{})
{
}
...
auto
QueueApplicationRunner::Start() -> base::Status
{
...
auto
QueueApplicationRunner::Stop() -> base::Status
{
...
auto
QueueApplicationRunner::running() const -> bool
{
