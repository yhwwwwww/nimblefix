...
auto ResolveQueueApplication(ApplicationCallbacks* application) -> base::Result<QueueApplication*> {
    ...
    if (provider == nullptr) {
        return base::Status::InvalidArgument(
            "managed queue runner requires application to implement QueueApplicationProvider");
    }
    return &provider->queue_application();
}

auto ValidateManagedQueueRunnerOptions(
    ...
    if (options.mode != ManagedQueueApplicationRunnerMode::kThreaded &&
        !options.thread_options.cpu_affinity.empty()) {
        return base::Status::InvalidArgument(
            "queue application runner cpu_affinity requires threaded mode");
    }
    if (options.thread_options.cpu_affinity.size() > expected_worker_count) {
        return base::Status::InvalidArgument(
            "queue application runner cpu_affinity must not contain more entries than runtime worker_count");
    }
...
auto Engine::LoadProfiles(const EngineConfig& config) -> base::Status {
    profiles_.Clear();
    ...
    for (const auto& dict_paths : config.profile_dictionaries) {
        ...
        trace_.Record(
            ...
            profile_id,
            0U,
            dict_paths.front().string());
    }

    return base::Status::Ok();
}

auto Engine::Boot(const EngineConfig& config) -> base::Status {
...
auto Engine::EnsureManagedQueueRunnerStarted(
    const void* owner,
    ...
    if (options == nullptr || !options->has_value()) {
        return base::Status::InvalidArgument(
            "managed queue runner requires runner options before first start");
    }
    ...
    if (options->value().mode == ManagedQueueApplicationRunnerMode::kThreaded) {
        auto runner = std::make_unique<QueueApplicationRunner>(
            queue_application.value(),
...
auto Engine::PollManagedQueueWorkerOnce(const void* owner, std::uint32_t worker_id)
...
auto Engine::StopManagedQueueRunner(const void* owner) -> base::Status {
    if (owner == nullptr) {
        return base::Status::InvalidArgument("managed queue runner requires an owner token");
    }
...
auto Engine::ReleaseManagedQueueRunner(const void* owner) -> base::Status {
    if (owner == nullptr) {
        return base::Status::InvalidArgument("managed queue runner requires an owner token");
    }
...
auto Engine::FindCounterpartyConfig(std::uint64_t session_id) const
...
auto Engine::FindListenerConfig(std::string_view name) const -> const ListenerConfig* {
...
auto Engine::LoadDictionaryView(std::uint64_t profile_id) const
    -> base::Result<profile::NormalizedDictionaryView> {
...
auto Engine::ResolveInboundSession(const codec::SessionHeader& header) const
    -> base::Result<ResolvedCounterparty> {
...
auto Engine::ResolveInboundSession(const codec::SessionHeaderView& header) const
    -> base::Result<ResolvedCounterparty> {
    if (!config_.has_value()) {
    ...
    for (const auto& counterparty : config_->counterparties) {
        ...
        if (!counterparty.default_appl_ver_id.empty() &&
            counterparty.default_appl_ver_id != header.default_appl_ver_id) {
            continue;
        }
...
void Engine::SetSessionFactory(SessionFactory factory) {
...
void WhitelistSessionFactory::AllowAny(const CounterpartyConfig& config_template) {
