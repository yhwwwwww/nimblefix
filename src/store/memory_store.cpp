...
auto
AppendPayload(std::vector<std::byte>* payload_arena, std::span<const std::byte> payload)
...
auto
MakeStoredRecord(std::vector<std::byte>* payload_arena, const MessageRecordView& record)
  -> MemorySessionStore::StoredMessageRecord
{
...
auto
MaterializeRecord(std::uint64_t session_id,
                  ...
                  const MemorySessionStore::StoredMessageRecord& record) -> MessageRecord
{
  ...
  owned.payload.insert(owned.payload.end(),
                       payload_arena.begin() + static_cast<std::ptrdiff_t>(record.payload_offset),
                       payload_arena.begin() +
                         static_cast<std::ptrdiff_t>(record.payload_offset + record.payload_size));
  return owned;
}

auto
ViewRecord(std::uint64_t session_id,
           const std::vector<std::byte>& payload_arena,
           const MemorySessionStore::StoredMessageRecord& record) -> MessageRecordView
{
  return MessageRecordView{
    ...
    .timestamp_ns = record.timestamp_ns,
    .flags = record.flags,
    .payload = std::span<const std::byte>(payload_arena.data() + static_cast<std::ptrdiff_t>(record.payload_offset),
                                          record.payload_size),
    .body_start_offset = record.body_start_offset,
  };
}
...
auto
UpsertBySequence(std::vector<MemorySessionStore::StoredMessageRecord>& records,
                 std::vector<std::byte>* payload_arena,
                 const MessageRecordView& record) -> void
{
  auto it =
    std::lower_bound(records.begin(), records.end(), record.seq_num, [](const auto& existing, std::uint32_t seq_num) {
      return existing.seq_num < seq_num;
    });
...
auto
MemorySessionStore::SaveOutbound(const MessageRecord& record) -> base::Status
{
  return SaveOutboundView(record.view());
}

auto
MemorySessionStore::SaveOutboundView(const MessageRecordView& record) -> base::Status
{
...
auto
MemorySessionStore::SaveInbound(const MessageRecord& record) -> base::Status
{
  return SaveInboundView(record.view());
}

auto
MemorySessionStore::SaveInboundView(const MessageRecordView& record) -> base::Status
{
  auto status = ValidateRecordView(record);
  if (!status.ok()) {
    return status;
...
auto
MemorySessionStore::LoadOutboundRange(std::uint64_t session_id, std::uint32_t begin_seq, std::uint32_t end_seq) const
...
auto
MemorySessionStore::LoadOutboundRangeViews(std::uint64_t session_id,
...
auto
MemorySessionStore::SaveRecoveryState(const SessionRecoveryState& state) -> base::Status
{
...
auto
MemorySessionStore::SaveInboundViewAndRecoveryState(const MessageRecordView& record, const SessionRecoveryState& state)
  -> base::Status
{
  auto status = ValidateRecordView(record);
  if (!status.ok()) {
...
auto
MemorySessionStore::LoadRecoveryState(std::uint64_t session_id) const -> base::Result<SessionRecoveryState>
{
  const auto it = sessions_.find(session_id);
...
auto
MemorySessionStore::ResetSession(std::uint64_t session_id) -> base::Status
{
...
auto
MemorySessionStore::Rollover() -> base::Status
{
  for (auto& [session_id, session] : sessions_) {
    // Rebuild payload arena with only live payloads from outbound + inbound
    // records.
