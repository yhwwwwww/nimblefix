...
auto
ValidateRecoveryState(const SessionRecoveryState& state) -> base::Status
{
...
auto
CloseSegment(SegmentHandle& segment) -> void
{
...
auto
HasMagic(const StoreFileHeader& header, const std::array<char, 8>& expected) -> bool
{
...
auto
ReadExact(int fd, void* data, std::size_t size, std::uint64_t offset) -> bool
{
  auto* bytes = static_cast<std::byte*>(data);
  std::size_t read = 0;
  while (read < size) {
    const auto result = ::pread(fd, bytes + read, size - read, static_cast<off_t>(offset + read));
    if (result < 0) {
      if (errno == EINTR) {
        continue;
      }
...
auto
EnsureDirectory(const std::filesystem::path& path) -> base::Status
{
  ...
  if (error) {
    return base::Status::IoError("unable to create durable batch store directory '" + path.string() +
                                 "': " + error.message());
  }
...
auto
BuildFileHeader(const std::array<char, 8>& magic, std::uint64_t segment_id) -> StoreFileHeader
{
  StoreFileHeader header{};
...
auto
SyncFileHeader(int fd, StoreFileHeader* header, std::uint64_t size) -> bool
{
...
auto
OpenStoreFile(const std::filesystem::path& path,
              const std::array<char, 8>& magic,
              std::uint64_t default_segment_id,
              std::optional<std::uint64_t> expected_segment_id = std::nullopt) -> base::Result<OpenedStoreFile>
{
  ...
  if (expected_segment_id.has_value() && opened.header.segment_id != expected_segment_id.value()) {
    CloseFd(opened.fd);
    return base::Status::FormatError("durable batch store file segment id does "
                                     "not match its companion file");
  }
...
auto
MakeSegmentFileName(std::uint64_t segment_id, std::string_view suffix) -> std::string
{
...
auto
ParseSegmentId(const std::filesystem::path& path) -> std::optional<std::uint64_t>
{
  ...
  const auto digits =
    filename.substr(kSegmentFilePrefix.size(), filename.size() - kSegmentFilePrefix.size() - kSegmentLogSuffix.size());
  try {
...
auto
OpenSegmentHandle(const std::filesystem::path& log_path,
                  const std::filesystem::path& index_path,
                  std::uint64_t default_segment_id,
                  std::optional<std::uint64_t> expected_segment_id = std::nullopt) -> base::Result<SegmentHandle>
{
...
auto
LoadSegmentIndex(const SegmentHandle& segment,
  ...
  -> base::Status
{
  ...
  while (offset < segment.index_size) {
    ...
    if (!ReadExact(segment.index_fd, &entry, sizeof(entry), offset)) {
      return base::Status::IoError(IoErrorMessage(segment.index_path, "unable to read durable batch index entry"));
    }
...
auto
LoadRecoveryLog(int fd,
                std::uint64_t file_size,
                const std::filesystem::path& path,
                std::unordered_map<std::uint64_t, SessionRecoveryState>* recovery_states) -> base::Status
{
  std::uint64_t offset = sizeof(StoreFileHeader);
...
auto
DetectActiveUtcDay(const SegmentHandle& segment, std::int64_t offset_ns = 0)
  -> base::Result<std::optional<std::int64_t>>
{
  std::uint64_t offset = sizeof(StoreFileHeader);
  ...
  while (offset < segment.log_size) {
    ...
    if (header.header_size != sizeof(StoreRecordHeader)) {
      return base::Status::FormatError("durable batch store log contains a "
                                       "record with an unexpected header size");
    }
...
auto
RemoveSegmentEntries(std::unordered_map<std::uint64_t, std::map<std::uint32_t, OutboundLocation>>* outbound_index,
...
auto
SyncDataFile(int fd) -> base::Status
{
  if (::fdatasync(fd) != 0) {
...
auto
ResolveSegmentHandle(const DurableBatchSessionStore::Impl& impl, std::uint64_t segment_id) -> const SegmentHandle*
{
...
auto
VisitSegmentRecords(const SegmentHandle& segment, Fn&& fn) -> base::Status
{
  std::uint64_t offset = sizeof(StoreFileHeader);
  ...
  while (offset < segment.log_size) {
    ...
    if (header.payload_size != 0U &&
        !ReadExact(segment.log_fd, payload.data(), header.payload_size, offset + sizeof(StoreRecordHeader))) {
...
DurableBatchSessionStore::DurableBatchSessionStore(std::filesystem::path root, DurableBatchStoreOptions options)
  : root_(std::move(root))
  ...
  , impl_(std::make_unique<Impl>())
{
  ...
  impl_->pending_payload_arena.reserve(
    std::max<std::size_t>(kPendingPayloadArenaFloorBytes, options_.flush_threshold * kPendingPayloadBytesPerEntry));
}
...
auto
DurableBatchSessionStore::Open() -> base::Status
{
  ...
  for (const auto& entry : std::filesystem::directory_iterator(archive_dir, iter_error)) {
    if (iter_error) {
      return base::Status::IoError("unable to enumerate durable batch archive directory '" + archive_dir.string() +
                                   "': " + iter_error.message());
    }
  ...
  for (const auto segment_id : archive_ids) {
    auto segment = OpenSegmentHandle(archive_dir / MakeSegmentFileName(segment_id, ".log"),
  ...
  if (exists_error) {
    return base::Status::IoError("unable to inspect durable batch active log path '" + active_log_path.string() +
                                 "': " + exists_error.message());
  }
  const bool active_index_exists = std::filesystem::exists(active_index_path, exists_error);
  if (exists_error) {
    return base::Status::IoError("unable to inspect durable batch active index path '" + active_index_path.string() +
                                 "': " + exists_error.message());
  }
  ...
  auto active = OpenSegmentHandle(active_log_path,
                                  active_index_path,
                                  default_segment_id,
                                  active_log_exists ? std::optional<std::uint64_t>{}
                                                    : std::optional<std::uint64_t>{ default_segment_id });
  if (!active.ok()) {
  ...
  status = LoadRecoveryLog(impl_->recovery_fd, impl_->recovery_size, impl_->recovery_path, &impl_->recovery_states);
  if (!status.ok()) {
    return status;
  }
  ...
  if (options_.max_archived_segments != 0U) {
    while (impl_->archived_segments.size() > options_.max_archived_segments) {
      ...
      if (remove_error) {
        return base::Status::IoError("unable to prune archived durable store log '" + oldest->second.log_path.string() +
                                     "': " + remove_error.message());
      }
      std::filesystem::remove(oldest->second.index_path, remove_error);
      if (remove_error) {
        return base::Status::IoError("unable to prune archived durable store index '" +
                                     oldest->second.index_path.string() + "': " + remove_error.message());
      }
...
auto
DurableBatchSessionStore::AppendMessage(bool outbound, const MessageRecord& record) -> base::Status
{
...
auto
DurableBatchSessionStore::AppendMessageView(bool outbound, const MessageRecordView& record) -> base::Status
{
  ...
  if (options_.rollover_mode == DurableStoreRolloverMode::kUtcDay) {
    ...
    const bool active_has_messages =
      impl_->active_segment.has_records ||
      std::any_of(impl_->pending_entries.begin(), impl_->pending_entries.end(), [](const PendingEntry& entry) {
  ...
  impl_->pending_payload_arena.insert(impl_->pending_payload_arena.end(), record.payload.begin(), record.payload.end());
...
auto
DurableBatchSessionStore::QueueRecoveryState(const SessionRecoveryState& state) -> base::Status
{
...
auto
DurableBatchSessionStore::Flush() -> base::Status
{
  auto status = Open();
  if (!status.ok()) {
    return status;
  ...
  if (!ReadExact(impl_->recovery_fd, &recovery_header, sizeof(recovery_header), 0U)) {
    return base::Status::IoError(IoErrorMessage(impl_->recovery_path, "unable to read durable batch recovery header"));
  }
  ...
  for (const auto& entry : impl_->pending_entries) {
    if (entry.kind == PendingEntryKind::kRecovery) {
      ...
      if (!WriteExact(impl_->recovery_fd, &record, sizeof(record), impl_->recovery_size)) {
        return base::Status::IoError(IoErrorMessage(impl_->recovery_path, "unable to append durable recovery record"));
      }
    ...
    StoreRecordHeader header{};
    header.record_type = static_cast<std::uint32_t>(outbound ? StoreRecordType::kOutbound : StoreRecordType::kInbound);
    ...
    if (entry.payload_size != 0U && !WriteExact(impl_->active_segment.log_fd,
    ...
    if (outbound) {
      ...
      if (!WriteExact(
            impl_->active_segment.index_fd, &index_entry, sizeof(index_entry), impl_->active_segment.index_size)) {
        return base::Status::IoError(
  ...
  if (wrote_recovery && !SyncFileHeader(impl_->recovery_fd, &recovery_header, impl_->recovery_size)) {
    return base::Status::IoError(IoErrorMessage(impl_->recovery_path, "unable to update durable recovery header"));
  }
...
auto
DurableBatchSessionStore::Rollover() -> base::Status
{
  auto status = Flush();
  if (!status.ok()) {
    return status;
  ...
  if (rename_error) {
    return base::Status::IoError("unable to archive durable batch log '" + archived.log_path.string() +
  ...
  if (rename_error) {
    ...
    return base::Status::IoError("unable to archive durable batch index '" + archived.index_path.string() +
                                 "': " + rename_error.message());
  }

  auto reopened = OpenSegmentHandle(archived_log_path, archived_index_path, archived.segment_id, archived.segment_id);
  if (!reopened.ok()) {
  ...
  if (impl_->recovery_fd >= 0 && !impl_->recovery_states.empty()) {
    ...
    if (fd < 0) {
      return base::Status::IoError(IoErrorMessage(impl_->recovery_path, "unable to truncate recovery log"));
    }
    auto rec_header = BuildFileHeader(kRecoveryMagic, 0U);
    if (!WriteExact(fd, &rec_header, sizeof(rec_header), 0U)) {
      CloseFd(fd);
      return base::Status::IoError(IoErrorMessage(impl_->recovery_path, "unable to write recovery log header"));
    }
    std::uint64_t offset = sizeof(StoreFileHeader);
    for (const auto& [sid, state] : impl_->recovery_states) {
      ...
      if (!WriteExact(fd, &record, sizeof(record), offset)) {
        CloseFd(fd);
        return base::Status::IoError(IoErrorMessage(impl_->recovery_path, "unable to write compacted recovery record"));
      }
      offset += sizeof(record);
    }
    if (!SyncFileHeader(fd, &rec_header, offset)) {
      CloseFd(fd);
      return base::Status::IoError(IoErrorMessage(impl_->recovery_path, "unable to update compacted recovery header"));
    }
...
auto
DurableBatchSessionStore::SaveOutbound(const MessageRecord& record) -> base::Status
{
  return AppendMessage(true, record);
}

auto
DurableBatchSessionStore::SaveInbound(const MessageRecord& record) -> base::Status
{
  return AppendMessage(false, record);
}

auto
DurableBatchSessionStore::SaveOutboundView(const MessageRecordView& record) -> base::Status
{
  return AppendMessageView(true, record);
}

auto
DurableBatchSessionStore::SaveInboundView(const MessageRecordView& record) -> base::Status
{
  return AppendMessageView(false, record);
}

auto
DurableBatchSessionStore::LoadOutboundRange(std::uint64_t session_id,
                                            ...
                                            std::uint32_t end_seq) const -> base::Result<std::vector<MessageRecord>>
{
  ...
  auto status =
    impl_->pending_entries.empty() ? base::Status::Ok() : const_cast<DurableBatchSessionStore*>(this)->Flush();
  if (!status.ok()) {
    return status;
  }
  ...
  for (auto it = session_it->second.lower_bound(begin_seq); it != session_it->second.end() && it->first <= end_seq;
    ...
    if (!ReadExact(segment->log_fd, &header, sizeof(header), it->second.log_offset)) {
      return base::Status::IoError(IoErrorMessage(segment->log_path, "unable to read durable outbound record"));
    }
    if (header.record_type != static_cast<std::uint32_t>(StoreRecordType::kOutbound) ||
        header.header_size != sizeof(StoreRecordHeader)) {
    ...
    if (header.reserved0 > header.payload_size) {
      return base::Status::FormatError("durable batch store outbound record "
                                       "has an invalid body_start_offset");
    }
    ...
    if (header.payload_size != 0U && !ReadExact(segment->log_fd,
                                                record.payload.data(),
                                                header.payload_size,
                                                it->second.log_offset + sizeof(StoreRecordHeader))) {
      return base::Status::IoError(IoErrorMessage(segment->log_path, "unable to read durable outbound payload"));
    }
...
auto
DurableBatchSessionStore::LoadOutboundRangeViews(std::uint64_t session_id,
                                                 std::uint32_t begin_seq,
                                                 std::uint32_t end_seq) const -> base::Result<MessageRecordViewRange>
{
  ...
  for (const auto& entry : indexed) {
    if (entry.payload_size != 0U && !ReadExact(entry.segment->log_fd,
                                               records.payload_storage.data() + payload_offset,
                                               entry.payload_size,
                                               entry.log_offset + sizeof(StoreRecordHeader))) {
      return base::Status::IoError(IoErrorMessage(entry.segment->log_path, "unable to read durable outbound payload"));
    }
...
auto
DurableBatchSessionStore::LoadRecoveryState(std::uint64_t session_id) const -> base::Result<SessionRecoveryState>
{
  auto status =
    impl_->pending_entries.empty() ? base::Status::Ok() : const_cast<DurableBatchSessionStore*>(this)->Flush();
  if (!status.ok()) {
...
auto
DurableBatchSessionStore::Refresh() -> base::Status
{
  CloseSegment(impl_->active_segment);
  impl_->active_segment = {};
  for (auto& [segment_id, segment] : impl_->archived_segments) {
...
auto
DurableBatchSessionStore::ResetSession(std::uint64_t session_id) -> base::Status
{
  if (session_id == 0U) {
  ...
  if (error) {
    return base::Status::IoError("unable to prepare temporary durable batch store root '" + rebuilt_root.string() +
                                 "': " + error.message());
  }

  {
    ...
    status = VisitSegmentRecords(
      impl_->active_segment, [&](const StoreRecordHeader& header, std::span<const std::byte> payload) {
        if (header.session_id == session_id) {
          return base::Status::Ok();
        }
  ...
  if (error) {
    return base::Status::IoError("unable to reset durable batch store root '" + root_.string() +
                                 "': " + error.message());
  }

  std::filesystem::rename(rebuilt_root, root_, error);
  if (error) {
    return base::Status::IoError("unable to replace durable batch store root '" + root_.string() +
                                 "': " + error.message());
  }
