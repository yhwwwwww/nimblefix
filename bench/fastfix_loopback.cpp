...
//
// Two-process FIX loopback latency benchmark: genuine acceptor process vs.
// initiator process. Forks the current binary: the child becomes the acceptor,
// the parent becomes the initiator.
//
...
//   session thread  – owns all AdminProtocol state; drains a send queue (built
//   by
//                     sender), calls SendApplication + BusySend; also receives
//                     all inbound frames via TryReceiveFrameView, calls
//                     OnInbound, sends admin responses (heartbeats etc.), and
//                     records RTTs.
//   sender thread   – builds business structs → fastfix::message::Message,
//   records T0,
//                     pushes to the send queue, then sleeps send_interval_us.
//                     No protocol state touched here.
//
...
auto
BuildNewOrder(const NewOrderFields& req) -> fastfix::message::Message
{
...
auto
BuildCancelOrder(const CancelOrderFields& req) -> fastfix::message::Message
{
...
auto
BuildExecReportNew(fastfix::message::MessageView order, std::uint32_t exec_id) -> fastfix::message::Message
{
...
auto
BuildExecReportCanceled(fastfix::message::MessageView cancel_req, std::uint32_t exec_id) -> fastfix::message::Message
{
...
auto
ParseExecReport(fastfix::message::MessageView view) -> ParsedAck
{
  ParsedAck ack;
...
auto
MakeAcceptorProtocolConfig(std::uint32_t profile_id) -> fastfix::session::AdminProtocolConfig
{
  return fastfix::session::AdminProtocolConfig{
    .session =
      fastfix::session::SessionConfig{
        .session_id = 1U,
        .key = fastfix::session::SessionKey{ std::string(kBeginString),
...
auto
MakeInitiatorProtocolConfig(std::uint32_t profile_id) -> fastfix::session::AdminProtocolConfig
{
  return fastfix::session::AdminProtocolConfig{
    .session =
      fastfix::session::SessionConfig{
            ...
            std::uint32_t warmup)
{
  ...
  // Reserve storage proportional to the actual message count (2 messages per
  // pair, plus a small admin overhead).
  ...
  fastfix::session::AdminProtocol protocol{ MakeAcceptorProtocolConfig(dictionary.profile().header().profile_id),
                                            dictionary,
                                            &store };
...
auto
RunInitiator(std::uint16_t port,
             const fastfix::profile::NormalizedDictionaryView& dictionary,
             std::uint32_t iterations,
             std::uint32_t warmup,
             std::uint32_t send_interval_us) -> LoopbackResult
{
  ...
  auto conn = fastfix::transport::TcpConnection::Connect("127.0.0.1", port, std::chrono::seconds(10));
  if (!conn.ok()) {
  ...
  const std::uint32_t total_pairs = warmup + iterations;
  fastfix::store::MemorySessionStore store;
  store.ReserveAdditionalSessionStorage(2U, 2U * total_pairs + 32U, 2U * total_pairs + 32U, 2U * total_pairs * 1024U);

  fastfix::session::AdminProtocol protocol{ MakeInitiatorProtocolConfig(dictionary.profile().header().profile_id),
                                            dictionary,
                                            &store };
  ...
  // Send queue: main thread pushes built messages; session thread drains and
  // sends.
  struct SendRequest
  {
  ...
  // Results written exclusively by session thread; read by main thread after
  // join().
  ...
  // --- No mutex needed here — protocol and conn are touched only by this
  // thread.
  std::thread session_th([&] {
    ...
    while (acks_done < total_msgs) {
      // 1. Drain send queue: grab batch, release lock, then call protocol +
      // send.
      ...
      for (const auto& app : ev.value().application_messages) {
        ...
        if (it == pending.end())
          continue;
        const auto rtt_ns =
          static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - it->second).count());
        const bool is_cancel = (!ack.cl_ord_id.empty() && ack.cl_ord_id[0] == 'C');
        pending.erase(it);
        if (is_cancel)
          ocr_rtts.push_back(rtt_ns);
        else
          nos_rtts.push_back(rtt_ns);
        ++acks_done;
      }

      if (ev.value().disconnect)
        break;
    }
  });
  ...
  if (nos_rtts.size() > warmup) {
    result.nos_rtts.assign(nos_rtts.begin() + static_cast<std::ptrdiff_t>(warmup), nos_rtts.end());
  }
  if (ocr_rtts.size() > warmup) {
    result.ocr_rtts.assign(ocr_rtts.begin() + static_cast<std::ptrdiff_t>(warmup), ocr_rtts.end());
  }
  return result;
}
...
main(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--artifact" && i + 1 < argc) {
      ...
      std::fprintf(stderr,
                   "usage: fastfix-loopback --artifact <path> [--iterations N] "
  ...
  // Load dictionary before fork so both processes share a warm read-only
  // mapping.
  auto profile = fastfix::profile::LoadProfileArtifact(artifact_path);
  if (!profile.ok()) {
    std::fprintf(stderr,
                 "error: failed to load artifact '%s': %s\n",
                 artifact_path.c_str(),
  ...
  std::printf("FastFix two-process loopback: %u iterations per message type, "
