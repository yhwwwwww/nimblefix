...
//                     records RTTs.
//   sender thread   – builds business structs → nimble::message::Message,
//   records T0,
...
BuildNewOrder(const NewOrderFields& req) -> nimble::message::Message
{
  ...
  order.reserve_fields(16U)
    ...
    .set_char(fix44::Tag::SecurityIDSource, req.security_id_source)
...
auto
MakeAcceptorProtocolConfig(std::uint32_t profile_id) -> nimble::session::AdminProtocolConfig
{
  return nimble::session::AdminProtocolConfig{
    .session =
      nimble::session::SessionConfig{
        .session_id = 1U,
        .key = nimble::session::SessionKey{ std::string(kBeginString),
...
auto
MakeInitiatorProtocolConfig(std::uint32_t profile_id) -> nimble::session::AdminProtocolConfig
{
  return nimble::session::AdminProtocolConfig{
    .session =
      nimble::session::SessionConfig{
            ...
            std::uint32_t warmup)
{
  ...
  auto listener = nimble::transport::TcpAcceptor::Listen("127.0.0.1", 0U);
  if (!listener.ok()) {
  ...
  nimble::session::AdminProtocol protocol{ MakeAcceptorProtocolConfig(dictionary.profile().header().profile_id),
                                           dictionary,
                                           &store };
  ...
  while (true) {
    ...
    auto decoded = nimble::codec::DecodeFixMessageView(raw_frame.value(), dictionary);
    if (!decoded.ok())
      continue;
             ...
             std::uint32_t send_interval_us) -> LoopbackResult
{
  ...
  auto conn = nimble::transport::TcpConnection::Connect("127.0.0.1", port, std::chrono::seconds(10));
  if (!conn.ok()) {
  ...
  nimble::session::AdminProtocol protocol{ MakeInitiatorProtocolConfig(dictionary.profile().header().profile_id),
                                           dictionary,
                                           &store };
  ...
  while (!session_active) {
    ...
    auto decoded = nimble::codec::DecodeFixMessageView(raw.value(), dictionary);
    if (!decoded.ok()) {
      return {};
    }
...
main(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--artifact" && i + 1 < argc) {
      ...
      std::fprintf(stderr,
                   "usage: nimblefix-loopback --artifact <path> [--iterations N] "
  ...
  std::printf("NimbleFIX two-process loopback: %u iterations per message type, "
  ...
  std::printf("=== NimbleFIX two-process loopback latency ===\n");
