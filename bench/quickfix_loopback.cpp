...
//
// Two-process FIX loopback latency benchmark: QuickFIX acceptor process vs. initiator process.
// Forks the current binary: the child becomes the ThreadedSocketAcceptor,
...
//
// QuickFIX is inherently multi-threaded: send via FIX::Session::sendToTarget() from
// any thread; acks arrive on the QuickFIX IO thread via Application::fromApp().
//
// Measurement boundary:
//   T0 = before struct → FIX44::NewOrderSingle or FIX44::OrderCancelRequest build
//   T1 = inside fromApp() callback, after ExecutionReport → struct extraction
//
// Usage:
...
auto BuildQFNewOrder(const NewOrderFields& req) -> FIX44::NewOrderSingle {
...
auto BuildQFCancelOrder(const CancelOrderFields& req) -> FIX44::OrderCancelRequest {
    FIX44::OrderCancelRequest cancel(
        FIX::OrigClOrdID(req.orig_cl_ord_id),
        FIX::ClOrdID(req.cl_ord_id),
        FIX::Side(req.side),
        FIX::TransactTime(FIX::UtcTimeStamp::now()));
    ...
    // SecurityExchange is in the Instrument component which is valid for OrderCancelRequest.
...
auto ParseQFExecReport(const FIX::Message& msg) -> ParsedAck {
    ParsedAck ack;
  ...
  public:
    ...
    void fromApp(const FIX::Message& msg, const FIX::SessionID& session_id)
        ...
        if (msg_type.getValue() == "D") {
            ...
            if (msg.isSetField(FIX::FIELD::ClOrdID))   msg.getField(cl_ord_id);
            if (msg.isSetField(FIX::FIELD::Side))       msg.getField(side);
            if (msg.isSetField(FIX::FIELD::OrderQty))   msg.getField(order_qty);
            ...
            FIX::OrderQty order_qty;
            if (msg.isSetField(FIX::FIELD::ClOrdID))     msg.getField(cl_ord_id);
            if (msg.isSetField(FIX::FIELD::OrigClOrdID)) msg.getField(orig_cl_ord_id);
            if (msg.isSetField(FIX::FIELD::Side))        msg.getField(side);
            if (msg.isSetField(FIX::FIELD::OrderQty))    msg.getField(order_qty);
...
class QFInitiatorApp final : public FIX::Application {
  public:
    ...
    void recordSend(const std::string& cl_ord_id,
    ...
    void fromApp(const FIX::Message& msg, const FIX::SessionID&)
        ...
        const std::uint64_t rtt = static_cast<std::uint64_t>(
            std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count());
        {
            std::lock_guard<std::mutex> lk(results_mutex);
            if (is_cancel) ocr_rtts.push_back(rtt);
            else            nos_rtts.push_back(rtt);
        }
...
static auto FindEphemeralPort() -> int {
    const int sock = ::socket(AF_INET, SOCK_STREAM, 0);
    if (sock < 0) return 19956;
    sockaddr_in addr{};
...
static auto MakeAcceptorSettings(const std::filesystem::path& xml_path, int port) -> FIX::SessionSettings {
    const std::string cfg =
        ...
        "DataDictionary=" + xml_path.string() + "\n"
        "[SESSION]\n"
        ...
        "TargetCompID=BUY\n"
        "HeartBtInt=30\n"
        "SocketAcceptPort=" + std::to_string(port) + "\n";
    std::istringstream stream(cfg);
    return FIX::SessionSettings(stream);
}

static auto MakeInitiatorSettings(const std::filesystem::path& xml_path, int port) -> FIX::SessionSettings {
    const std::string cfg =
        "[DEFAULT]\n"
        "NonStopSession=Y\n"
        "PersistMessages=N\n"
        ...
        "SocketConnectPort=" + std::to_string(port) + "\n"
        "ReconnectInterval=1\n";
    std::istringstream stream(cfg);
    return FIX::SessionSettings(stream);
}
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--xml" && i + 1 < argc) {
            ...
            std::fprintf(stderr,
                "usage: quickfix-loopback --xml <FIX44.xml> [--iterations N] [--warmup N] [--send-interval-us N]\n");
            return 1;
        }
    }
    ...
    // Pipe: acceptor writes 1 byte when started, initiator reads before connecting.
    int pipe_ready[2];
    if (::pipe(pipe_ready) != 0) { std::perror("pipe"); return 1; }
    ...
    std::printf("QuickFIX two-process loopback: %u iterations per message type, %u warmup  send-interval=%u us\n",
    ...
    // Wait for all acks (fromApp callback runs on the QuickFIX IO thread independently).
    ...
    // Trim warmup samples from collected RTTs (they were mixed in via fromApp, so slice)
    auto trim_warmup = [warmup](std::vector<std::uint64_t>& v) {
        // warmup samples are the first 'warmup' entries recorded; we recorded in order,
        // so erase the first min(warmup, v.size()) entries.
        if (v.size() > warmup) {
