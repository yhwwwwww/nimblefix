...
main(int argc, char** argv)
{
  ...
  if (xml_mode) {
    ...
    try {
      nfd_text = nimble::tools::ConvertXmlToNfd(xml_content, profile_id);
  ...
  if (need_dictionary) {
    if (xml_mode) {
      // Use the .nfd text we just generated
      auto parsed = nimble::profile::LoadNormalizedDictionaryText(nfd_text);
      if (!parsed.ok()) {
      ...
      // Load and merge input .nfd files
