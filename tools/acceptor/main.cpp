...
auto PrintUsage() -> void {
    std::cout << "usage: fastfix-acceptor (--artifact <profile.art> --bind <host> --port <port> --sender <sender> --target <target> [--begin-string <value>] [--default-appl-ver-id <value>] [--dispatch-mode inline|queue] [--validation-mode strict|compatible|permissive|raw-pass-through] [--front-door-cpu <id>] [--worker-cpus <csv>] [--queue-runner-mode co-scheduled|threaded] [--app-cpus <csv>] | --config <engine.ffcfg> [--listener <name>] [--front-door-cpu <id>] [--worker-cpus <csv>] [--queue-runner-mode co-scheduled|threaded] [--app-cpus <csv>]) [--max-sessions <count>] [--idle-timeout-ms <value>]\n";
}
...
auto MakeDirectCounterparty(const Options& options, std::uint64_t profile_id) -> fastfix::runtime::CounterpartyConfig {
...
auto ParseDispatchMode(std::string_view token) -> std::optional<fastfix::runtime::AppDispatchMode> {
    if (token == "inline") {
...
auto ParseValidationMode(std::string_view token) -> std::optional<fastfix::session::ValidationMode> {
    if (token == "strict") {
...
auto ParseCpuList(std::string_view token) -> std::optional<std::vector<std::uint32_t>> {
...
auto ParseQueueAppMode(std::string_view token) -> std::optional<fastfix::runtime::QueueAppThreadingMode> {
...
auto ResolveManagedQueueRunnerMode(
    fastfix::runtime::QueueAppThreadingMode mode) -> fastfix::runtime::ManagedQueueApplicationRunnerMode {
...
auto CollectQueueSessions(const fastfix::runtime::EngineConfig& config) -> std::unordered_set<std::uint64_t> {
...
class ToolApplication final : public fastfix::runtime::ApplicationCallbacks,
...
int main(int argc, char** argv) {
    ...
    fastfix::runtime::LiveAcceptor acceptor(
        &engine,
