...
auto Fnv1a64(const std::string& data) -> std::uint64_t {
...
auto MapXmlType(const std::string& xml_type) -> std::string {
    if (xml_type == "STRING" || xml_type == "MULTIPLEVALUESTRING" ||
        xml_type == "MULTIPLECHARVALUE" || xml_type == "MULTIPLESTRINGVALUE" ||
        xml_type == "DATA" || xml_type == "XMLDATA" ||
        xml_type == "EXCHANGE" || xml_type == "COUNTRY" || xml_type == "CURRENCY" ||
        xml_type == "LANGUAGE" || xml_type == "TZTIMEONLY" || xml_type == "TZTIMESTAMP") {
        return "string";
    }
    if (xml_type == "INT" || xml_type == "LENGTH" ||
        xml_type == "SEQNUM" || xml_type == "NUMINGROUP" ||
    ...
    if (xml_type == "PRICE" || xml_type == "QTY" || xml_type == "FLOAT" ||
        xml_type == "PRICEOFFSET" || xml_type == "PERCENTAGE" || xml_type == "AMT") {
        return "float";
    }
    if (xml_type == "BOOLEAN") {
        return "boolean";
    }
    if (xml_type == "UTCTIMESTAMP" || xml_type == "UTCDATE" ||
        xml_type == "UTCDATEONLY" || xml_type == "UTCTIMEONLY" ||
...
auto ConvertXmlToFfd(const std::string& xml_content, std::uint64_t profile_id) -> std::string {
    ...
    std::sort(sorted_fields.begin(), sorted_fields.end(),
              [](const FieldInfo& a, const FieldInfo& b) { return a.tag < b.tag; });
    ...
    // 5. Also process header/trailer for group definitions (but don't emit them as messages).
