...
auto PrintUsage() -> void {
    std::cout << "usage: fastfix-soak --profile <profile.art> [--workers N] [--sessions N] [--iterations N]"
                 " [--gap-every N] [--duplicate-every N] [--replay-every N] [--reorder-every N]"
...
int main(int argc, char** argv) {
    ...
    std::cout << "soak completed: iterations=" << report.value().iterations_completed
              ...
              << " resend=" << report.value().total_resend_requests
              << " gapfill=" << report.value().total_gap_fills
              << " replay=" << report.value().total_replay_requests
              << " reorder=" << report.value().total_reorder_events
              << " drop=" << report.value().total_drop_events
              << " jitter=" << report.value().total_jitter_events
              << " reconnect=" << report.value().total_reconnects
              << " duplicate=" << report.value().total_duplicate_inbound_messages
              << " timer=" << report.value().total_timer_events
              << " app=" << report.value().total_application_messages
