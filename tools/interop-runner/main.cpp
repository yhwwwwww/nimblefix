...
int main(int argc, char** argv) {
    ...
    std::cout << "interop scenario passed with "
              << report.value().sessions.size() << " sessions, "
              << report.value().metrics.sessions.size() << " metric entries, and "
              << report.value().trace_events.size() << " trace events\n";
    return 0;
}