...
auto PrintUsage() -> void {
    std::cout << "usage: fastfix-fuzz-codec --artifact <profile.art> --input <file-or-directory>"
                 " [--mode codec|admin]\n";
}
...
auto ProcessAdminInput(
    std::string text,
    const fastfix::profile::NormalizedDictionaryView& dictionary,
    FuzzStats* stats) -> fastfix::base::Status {
...
int main(int argc, char** argv) {
    ...
    if (mode == FuzzMode::kCodec) {
        std::cout << "processed " << files.size() << " wire inputs, peeked " << stats.peeked
                  << ", decoded " << stats.decoded << ", flagged " << stats.flagged << '\n';
        return 0;
    }

    std::cout << "processed " << files.size() << " admin corpora, peeked " << stats.peeked
              << ", decoded " << stats.decoded
              << ", flagged " << stats.flagged
              << ", app-input " << stats.application_inputs
              << ", events " << stats.admin_events
              << ", app " << stats.application_messages
              << ", outbound " << stats.outbound_frames
              << ", timers " << stats.timer_callbacks
              << ", errors " << stats.admin_errors << '\n';
    return 0;
}