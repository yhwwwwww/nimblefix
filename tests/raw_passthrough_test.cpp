...
auto
ToStringView(std::span<const std::byte> bytes) -> std::string_view
{
...
TEST_CASE("raw-passthrough decode extracts session fields and raw body", "[raw-passthrough]")
{
  ...
  // The raw_body should contain the application fields:
  // 11=ORD-1|55=AAPL|54=1|38=100|
...
TEST_CASE("raw-passthrough forwarding rewrites sender and target sub ids as "
          "header fields",
          "[raw-passthrough]")
{
  const auto frame =
...
TEST_CASE("raw-passthrough replay preserves extended header semantics across "
          "encode paths",
          "[raw-passthrough]")
{
  const auto frame =
