...
TEST_CASE("resend-recovery", "[resend-recovery]")
{
  ...
  REQUIRE(store
            ...
            .ok());
  REQUIRE(fastfix::session::RecoverSession(session, store, fastfix::session::RecoveryMode::kWarmRestart).ok());
...

  {
    ...
    REQUIRE(
      fastfix::session::RecoverSession(durable_session, durable_store, fastfix::session::RecoveryMode::kWarmRestart)
        .ok());
