...
inline auto LoadFix44DictionaryView()
...
inline auto LoadFix44DictionaryViewOrSkip()
    -> profile::NormalizedDictionaryView {
...
inline auto BuildDictionaryViewFromDictionary(profile::NormalizedDictionary dict)
    -> base::Result<profile::NormalizedDictionaryView> {
    ...
    const auto artifact_path =
        std::filesystem::temp_directory_path() / "fastfix-test-support.art";
    const auto write_status =
        profile::WriteProfileArtifact(artifact_path, artifact.value());
    if (!write_status.ok()) {
        return write_status;
    }
