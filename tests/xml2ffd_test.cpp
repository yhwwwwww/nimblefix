...
TEST_CASE("xml2ffd converts QuickFIX XML to valid ffd", "[xml2ffd]") {
    ...
    SECTION("message definitions") {
        // 9 messages: Heartbeat, TestRequest, ResendRequest, Reject, SequenceReset, Logout, Logon, ExecutionReport, NewOrderSingle.
        CHECK(dict.messages.size() == 9U);

        // Check Heartbeat is admin.
        auto hb_it = std::find_if(dict.messages.begin(), dict.messages.end(),
            [](const auto& m) { return m.msg_type == "0"; });
        REQUIRE(hb_it != dict.messages.end());
        CHECK(hb_it->name == "Heartbeat");
        CHECK((hb_it->flags & 1U) == 1U);  // kAdmin

        // Check Logon is admin.
        auto logon_it = std::find_if(dict.messages.begin(), dict.messages.end(),
            [](const auto& m) { return m.msg_type == "A"; });
        REQUIRE(logon_it != dict.messages.end());
        CHECK(logon_it->name == "Logon");
        CHECK((logon_it->flags & 1U) == 1U);

        // Check NewOrderSingle is app.
        auto nos_it = std::find_if(dict.messages.begin(), dict.messages.end(),
            [](const auto& m) { return m.msg_type == "D"; });
        ...
        auto er_it = std::find_if(dict.messages.begin(), dict.messages.end(),
            [](const auto& m) { return m.msg_type == "8"; });
    ...
    SECTION("group definitions") {
        ...
        // Check Allocs group.
        auto allocs_it = std::find_if(dict.groups.begin(), dict.groups.end(),
            [](const auto& g) { return g.count_tag == 78U; });
