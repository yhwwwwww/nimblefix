...
auto MakeEvent(std::uint64_t session_id, std::uint32_t worker_id, std::string text)
    ...
    auto OnRuntimeEvent(const fastfix::runtime::RuntimeEvent& event) -> fastfix::base::Status override {
        ...
        if (event.handle.worker_id() != expected_worker_id_) {
            return fastfix::base::Status::InvalidArgument(
                "queue application runner delivered an event to the wrong worker handler");
        }
        return fastfix::base::Status::Ok();
    }
...
TEST_CASE("application-poller", "[application-poller]") {
    ...
    fastfix::runtime::QueueApplicationPoller poller(
        &queue,
        &handler,
    ...
    auto worker_future = std::async(std::launch::async, [&]() {
        return poller.RunWorker(1U, stop_requested);
    });
    ...
    fastfix::runtime::QueueApplicationPoller single_worker_poller(
        &single_worker_queue,
    ...
    auto single_worker_future = std::async(std::launch::async, [&]() {
        return single_worker_poller.RunAll(single_stop_requested);
    });
    ...
    fastfix::runtime::QueueApplicationRunner runner(
        &runner_queue,
        std::move(handlers),
        fastfix::runtime::QueueApplicationPollerOptions{
            .max_events_per_poll = 8U,
    ...
    fastfix::runtime::QueueApplicationRunner affinity_runner(
        &affinity_queue,
