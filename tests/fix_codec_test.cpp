...
TEST_CASE("fix-codec", "[fix-codec]")
{
  ...
  REQUIRE(std::equal(
    decoded_view.value().raw.begin(), decoded_view.value().raw.end(), encoded.value().begin(), encoded.value().end()));
  ...
  // Generic encode uses dictionary rule order: ClOrdID < Parties < Symbol,
  // extras last.
...
auto
BuildFrameWithBodyLength(std::string_view body_fields,
...
TEST_CASE("PrecompiledTemplateTable build, find, and encode", "[fix-codec][precompiled-table]")
{
  ...
  REQUIRE(std::equal(buffer_global.bytes().begin(),
  ...
  REQUIRE(std::equal(buffer_direct.bytes().begin(),
