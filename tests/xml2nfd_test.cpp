...
#include "../tools/xml2nfd/xml2nfd.h"
...
TEST_CASE("xml2nfd converts QuickFIX XML to valid nfd", "[xml2nfd]")
{
  ...
  const auto nfd_text = nimble::tools::ConvertXmlToNfd(xml_content, 2001);
  REQUIRE(!nfd_text.empty());

  // Verify the output is valid nfd by loading it.
