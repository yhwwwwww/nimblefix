...
auto MakeResendProcessing() -> SessionCore {
    auto s = MakeActive();
    (void)s.BeginResend(2U, 5U);
    return s;
}

auto MakeAwaitingLogout() -> SessionCore {
    auto s = MakeActive();
    (void)s.BeginLogout();
    return s;
}
...
TEST_CASE("session-core: ResendProcessing -> AwaitingLogout (CompleteResend restores resume)", "[session-core][legal]") {
    // Enter resend from AwaitingLogout, then CompleteResend should return to AwaitingLogout
    auto s = MakeAwaitingLogout();
    REQUIRE(s.BeginResend(1U, 2U).ok());
    REQUIRE(s.state() == SessionState::kResendProcessing);
    REQUIRE(s.CompleteResend().ok());
...
TEST_CASE("session-core: AdvanceInboundExpectedSeq past resend end auto-completes", "[session-core][legal][gap]") {
    auto s = MakeActive();
    // Create gap: expect 1, see 4 → gap [1, 3], state = ResendProcessing, resume = Active
...
TEST_CASE("session-core: AdvanceInboundExpectedSeq within range stays ResendProcessing", "[session-core][legal][gap]") {
    auto s = MakeActive();
