...
auto
WriteUint64AtOffset(const std::filesystem::path& path, std::uint64_t offset, std::uint64_t value) -> void
{
...
auto
AppendBytes(const std::filesystem::path& path, std::string_view bytes) -> void
{
...
auto
CountArchivedLogs(const std::filesystem::path& root) -> std::size_t
{
...
auto
CanOpenReadWrite(const std::filesystem::path& path) -> bool
{
  const int fd = ::open(path.c_str(), O_RDWR);
  if (fd < 0) {
    return false;
...
auto
ReadOnlyFilesBlockReadWriteOpen(const std::filesystem::path& root) -> bool
{
  ...
  for (const auto& entry : std::filesystem::recursive_directory_iterator(root)) {
    if (entry.is_regular_file()) {
      std::filesystem::permissions(
        entry.path(), std::filesystem::perms::owner_read, std::filesystem::perm_options::replace);
    }
  }
