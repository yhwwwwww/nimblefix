...
{
  auto view_result = fastfix::tests::LoadFix44DictionaryView();
  if (!view_result.ok()) {
  ...
  const auto* msg_type = view.find_field(fastfix::codec::tags::kMsgType);
  ...
  const auto* group = view.find_group(fastfix::codec::tags::kNoPartyIDs);
