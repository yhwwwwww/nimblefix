...
auto PopExpired(fastfix::runtime::TimerWheel* wheel, std::uint64_t now_ns) -> std::vector<std::uint64_t> {
...
TEST_CASE("timer-wheel", "[timer-wheel]") {
    SECTION("basic scheduling and expiration") {
        fastfix::runtime::TimerWheel wheel(
            fastfix::runtime::TimerWheelOptions{.tick_ns = 10U, .slot_count = 4U});
