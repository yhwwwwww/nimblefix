...
auto
MakeSocketPair() -> std::array<int, 2>
{
...
auto
SendRaw(int fd, const std::vector<std::byte>& bytes, std::size_t offset, std::size_t length) -> bool
{
...
TEST_CASE("transport-fault", "[transport-fault]")
{
  ...
  SECTION("split and coalesce")
  {
    ...
    first_batch.insert(
      first_batch.end(), second.begin(), second.begin() + static_cast<std::ptrdiff_t>(second.size() / 2U));
    ...
    second_batch.insert(
      second_batch.end(), second.begin() + static_cast<std::ptrdiff_t>(second.size() / 2U), second.end());
